CREATE OR REPLACE EDITIONABLE PACKAGE "LABCONF"."TVCONTAB" IS
   ...
   ---------------------------------
    ...
    PROCEDURE SP_INSERTA (datospf IN partefija, referencia IN LABCONF.nmcodeps.dep_refcon%TYPE, ctaconcepto IN LABCONF.nmloconc.con_ctaref%TYPE,
                         seietu IN LABCONF.nmloconc.con_porcen%TYPE, impcar IN LABCONF.nmlohism.his_import%TYPE, impabo IN LABCONF.nmlohism.his_import%TYPE, tipo IN LABCONF.tvwkpoli.pol_tipo%TYPE);
    PROCEDURE SP_FINIQUITO (empleado IN LABCONF.nmwkmovt.mov_keyemp%TYPE, periodo IN LABCONF.nmwkmovt.mov_keyper%TYPE);
    PROCEDURE SP_RECLASIFICA (proceso IN LABCONF.nmlohism.his_keypro%TYPE, periodo IN LABCONF.nmlohism.his_keyper%TYPE, keypol IN LABCONF.tvwkpoli.pol_keypol%TYPE);
    --------------------------------
    --funciones
END TVCONTAB;
/
CREATE OR REPLACE EDITIONABLE PACKAGE BODY "LABCONF"."TVCONTAB" IS
/* -- Variables Globales-------------------------------------------- */
    poliza 	partefija;  -- Campos que no varian en la poliza para un registro especifico
...
/* -------------------------------------------------------------------------- */
  ...
  commit;
  END SP_INSERTA;
 PROCEDURE SP_FINIQUITO (empleado IN LABCONF.nmwkmovt.mov_keyemp%TYPE, periodo IN LABCONF.nmwkmovt.mov_keyper%TYPE) IS
  ...
  -- procesa
    ...
    LOOP
       ...
       -- inserta registros de la poliza madre
        ...
        --Inserta Registros de la poliza hija si el departamento tiene una referencia contable con 52 caracteres
        IF length(reg.dep_refcon) = 52 THEN
          IF substr(reg.con_ctaref, 15, 8) <> '00000000' THEN
    ...
    IF proceso = 6 OR proceso = 120 OR proceso = 139 THEN
      ...
      INSERT INTO LABCONF.TVWKPOLI
            (POL_KEYEMP,POL_KEYPRO,POL_KEYCON,POL_CODACU,POL_DESCTA,         POL_CIA,POL_NEG,     POL_CTA,POL_SCTA,POL_CC,POL_ICIA,      POL_TOP,    POL_IETU,POL_IMPCAR,     POL_IMPABO,     POL_KEYCIA,POL_KEYPOL,POL_CVEBAN,POL_FORPAG,POL_KEYBEN,POL_COMFAM,POL_TIPO,POL_FECMOV)
      SELECT POL_KEYEMP,POL_KEYPRO,POL_KEYCON,POL_CODACU,'RECLASIFICA_CONF7' POL_DESCTA,ws_cia POL_CIA,'01' POL_NEG,POL_CTA,POL_SCTA,POL_CC,'000' POL_ICIA,'0' POL_TOP,POL_IETU,SUM(POL_IMPABO) POL_IMPCAR,SUM(POL_IMPCAR) POL_IMPABO,POL_KEYCIA,POL_KEYPOL,POL_CVEBAN,POL_FORPAG,POL_KEYBEN,POL_COMFAM,1 POL_TIPO,POL_FECMOV
      FROM LABCONF.tvwkpoli
      WHERE pol_keypol = keypol
        AND pol_cia = '284'
        AND pol_cc NOT IN ('00000024','00000000')
        AND pol_keyemp = empleado
      GROUP BY POL_KEYEMP, POL_KEYPRO, POL_KEYCON, POL_CODACU,  POL_CTA, POL_SCTA, POL_CC,   POL_IETU, POL_KEYCIA, POL_KEYPOL, POL_CVEBAN, POL_FORPAG, POL_KEYBEN, POL_COMFAM, POL_FECMOV;
      INSERT INTO LABCONF.TVWKPOLI
            (POL_KEYEMP,POL_KEYPRO,POL_KEYCON,POL_CODACU,POL_DESCTA,         POL_CIA,POL_NEG,     POL_CTA,POL_SCTA,POL_CC,POL_ICIA,      POL_TOP,    POL_IETU,POL_IMPCAR,     POL_IMPABO,     POL_KEYCIA,POL_KEYPOL,POL_CVEBAN,POL_FORPAG,POL_KEYBEN,POL_COMFAM,POL_TIPO,POL_FECMOV)
     ...
     INSERT INTO LABCONF.TVWKPOLI
      ...
      INSERT INTO LABCONF.TVWKPOLI
            (POL_KEYEMP,POL_KEYPRO,POL_KEYCON,POL_CODACU,POL_DESCTA,         POL_CIA,POL_NEG,     POL_CTA,POL_SCTA,POL_CC,POL_ICIA,      POL_TOP,    POL_IETU,POL_IMPCAR,     POL_IMPABO,     POL_KEYCIA,POL_KEYPOL,POL_CVEBAN,POL_FORPAG,POL_KEYBEN,POL_COMFAM,POL_TIPO,POL_FECMOV)
      SELECT POL_KEYEMP,POL_KEYPRO,POL_KEYCON,POL_CODACU,'RECLASIFICA_CONF9' POL_DESCTA,ws_cia POL_CIA,'01' POL_NEG,POL_CTA,'121339' POL_SCTA,POL_CC,'000' POL_ICIA,'0' POL_TOP,POL_IETU,SUM(POL_IMPCAR) POL_IMPCAR,SUM(POL_IMPABO) POL_IMPABO,POL_KEYCIA,POL_KEYPOL,POL_CVEBAN,POL_FORPAG,POL_KEYBEN,POL_COMFAM,1 POL_TIPO,POL_FECMOV
      FROM LABCONF.tvwkpoli
      WHERE pol_keypol = keypol
        AND pol_cia = '284'
        AND pol_cc NOT IN ('00000024','00000000')
...
--FUNCIONES
  ...
  BEGIN
    ...
    INSERT INTO LABCONF.TVWKPOLI
      SELECT POL_KEYEMP,POL_KEYPRO,POL_KEYCON,POL_CODACU,'RECLASIFICA2',CEN_CIA,CEN_NEG,CEN_CTA,POL_SCTA,INC_KEYCEN,POL_ICIA,POL_TOP,POL_IETU,SUM(INC_IMPORT),0,POL_KEYCIA,POL_KEYPOL,POL_CVEBAN,POL_FORPAG,POL_KEYBEN,POL_COMFAM,POL_TIPO,POL_FECMOV
    ...
    FROM LABCONF.TVWKPOLI
      ...
      WHERE pol_keypro = inc_keypro AND inc_keyper = substr(pol_keypol,4,7) AND pol_keyemp = inc_keyemp AND pol_keycon = inc_keycon AND inc_keycen = cen_keycen
        AND inc_keypro = proceso
        AND inc_keyper = periodo
      GROUP BY POL_KEYEMP, POL_KEYPRO, POL_KEYCON, POL_CODACU, CEN_CIA,CEN_NEG,CEN_CTA, POL_SCTA, INC_KEYCEN, POL_ICIA, POL_TOP, POL_IETU, POL_KEYCIA, POL_KEYPOL, POL_CVEBAN, POL_FORPAG, POL_KEYBEN, POL_COMFAM, POL_TIPO, POL_FECMOV;
    COMMIT;
    */
    IF proceso = 6 OR proceso = 120 OR proceso = 139 THEN
      IF proceso = 6 OR proceso = 120 THEN
