create or replace procedure labconf.tvcontab_sp_finiquito (empleado labconf.nmwkmovt.mov_keyemp%type, periodo labconf.nmwkmovt.mov_keyper%type) as $body$
declare
...
--dmap moved type current package tvcontab;
nomina 	labconf.nmlohism.his_keynom%type;
--dmap moved type current package tvcontab;
...
--identifica proceso
select emp_keypro into strict proceso from labconf.nmcoempl
where emp_keyemp = empleado;
select per_keynom,per_keypol,pro_vercon
into strict nomina,keypol,cveversionproc
from labconf.nmloperi
inner join labconf.nmloproc on per_keypro = pro_keypro
where per_keypro = proceso
and per_keyper = periodo;
select case when nullif(vxn_keyver::text, '') is null then 0 else case when cveversionproc = 100 then vxn_keyver + 100 else vxn_keyver end end vxn_keyver  into strict cveversion
