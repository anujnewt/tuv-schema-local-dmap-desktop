create or replace procedure labconf."sp_nmlstpue"  (ws_nom_rep varchar, ws_ide_pcc varchar, wn_key_usu numeric, ws_key_men varchar, ws_hor_reg varchar) as $body$
declare
-- pgv moved types start
-- pgv moved types end
-- variables para la carga de la tabla de puestos.
ws_key_pue varchar(16);
ws_des_pue varchar(40);
ws_are_pue varchar(6);
ws_sub_are varchar(6);
-- variables para la carga de la tabla de tabuladores --
ws_key_sue varchar(4);
ws_cob_ert varchar(2);
ws_tip_tab varchar(2);
wn_sue_niv numeric(10);
wn_sub_niv numeric(10);
wd_fec_cad timestamp(0);
wn_sue_min decimal(12,2);
wn_sue_1qa decimal(12,2);
wn_sue_med decimal(12,2);
wn_sue_3qa decimal(12,2);
wn_sue_max decimal(12,2);
...
nullif(ran_keypue::text, '') is not null)
...
if (nullif(c_nmlstpue.pue_arepue::text, '') is not null) and (c_nmlstpue.pue_arepue != ' ') then
select pam_nompar into strict ws_des_are from glcopams
...
if (nullif(c_nmlstpue.pue_subare::text, '') is not null) and (c_nmlstpue.pue_subare != ' ') then
select pam_nompar into strict ws_des_sub from glcopams
