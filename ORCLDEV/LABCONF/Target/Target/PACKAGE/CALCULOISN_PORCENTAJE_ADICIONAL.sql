create or replace  function  labconf.calculoisn_porcentaje_adicional (ws_key_ent varchar,wn_por_adi numeric,ws_key_tab varchar,wn_base numeric, ws_key_cia varchar, wn_anio integer) returns numeric as $body$
declare
...
select case when nullif(pam_folini::text, '') is null then ent_poradi else coalesce(labconf.safe_to_number(pam_folini),0) end into strict wn_result
