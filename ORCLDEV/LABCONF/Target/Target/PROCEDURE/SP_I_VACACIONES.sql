create or replace procedure labconf."sp_i_vacaciones"  (o_num_r_procesados inout integer, o_num_r_insertados inout integer, o_num_r_actualizados inout integer, o_iregfijp inout integer, o_iregfijn inout integer) as $body$
declare
...
select pam_folfin into strict c_tab_vacaciones
...
begin
select tab_eledos into strict n_r_anio_trab from nmlotabn
where tab_keytab= trim(both c_tab_vacaciones) and tab_eledos >= n_anio_trabajados   order by  tab_eledos limit 1;
exception
...
from nmcocvac where vac_keyemp = n_keyemp and vac_antigu = n_anio_trabajados;
if n_existe = 0 then
...
where vac_keyemp = n_keyemp and vac_antigu = nagnostrab and vac_period = c_periodo;
