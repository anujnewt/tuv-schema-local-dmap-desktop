create or replace procedure labprod.interfacerecibos_recibostv2016_details ( v_keybin varchar, keypro numeric, keyper varchar, cv_empdetails inout refcursor ) as $body$
declare
-- pgv moved types end
/*keypro  proceso
keyper  periodo */
...
-----------------------------------------------------
begin
insert into labprod.ps_tpw_msgxconcs(select v_keypro, v_keyper, con_keycon , 1 from nmloconc where con_keycon in ( '001' ));
end;
-----------------------------------------------------
-- 2 tot_isr
-----------------------------------------------------
...
exception
when no_data_found then
insert into labprod.ps_tpw_msgxconcs(select v_keypro, v_keyper, con_keycon , 2 from nmloconc where con_keycon in ( '100','151','183','294','47A' )
);
end;
-----------------------------------------------------
-- 3 tot_per
...
exception
when no_data_found then
insert into labprod.ps_tpw_msgxconcs(select v_keypro, v_keyper, con_keycon, 3 from nmloconc  where con_keycon in ( '260','273','290' ));
end;
-----------------------------------------------------
-- 4 tot_ded
-----------------------------------------------------
