...
if nullif(ca2aux::text, '') is null or ca2aux = ' ' then
wn_res_pue :=  concat(wn_res_pue, '8') ;/* dmap converted statement end */
end if;
--1. buscar clientes
...
concat(wn_res_pue, ' prueba pasivos ahorro')) ;/* dmap converted statement end */
if wn_res_pue = ' ' then
call tvautsaf_local_sp_datosgenerales_pa_aut(ws_cve_cte, to_char(keyemp), recurp, nombre, paterno, materno, coalesce(domemp, ' '), coalesce(colemp, ' '),
