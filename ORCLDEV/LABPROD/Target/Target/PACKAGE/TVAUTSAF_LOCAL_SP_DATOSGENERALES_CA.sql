create or replace procedure labprod.tvautsaf_local_sp_datosgenerales_ca (recurp labprod.nmcoempl.emp_recurp%type, keyemp labprod.nmcoempl.emp_keyemp%type, nomemp labprod.nmcoempl.emp_nomemp%type, keypro labprod.nmcoempl.emp_keypro%type, cveban labprod.nmcoempl.emp_cveban%type, fecaux labprod.nmcoempl.emp_fecaux%type, domemp labprod.nmcoempl.emp_domemp%type, colemp labprod.nmcoempl.emp_colemp%type, munemp labprod.nmcoempl.emp_munemp%type, entemp labprod.nmcoempl.emp_entemp%type, codemp labprod.nmcoempl.emp_codemp%type, telemp labprod.nmcoempl.emp_telemp%type, cidemp labprod.nmcoempl.emp_cidemp%type, salmes labprod.nmcoempl.emp_salmes%type, keyloc labprod.nmcoempl.emp_keyloc%type, forpag labprod.nmcoempl.emp_forpag%type, ca2aux labprod.nmcoempl.emp_ca2aux%type, status labprod.nmcoempl.emp_status%type) as $body$
declare
...
materno := trim(both coalesce(sp_delimitador(nomemp,'/',2), ' '));/* dmap converted statement start */
...
concat(wn_res_pue, ' prueba caja ahorro CA')) ;/* dmap converted statement end */
if wn_res_pue = ' ' then
call tvautsaf_local_sp_datosgenerales_ca_aut(ws_cve_cte, to_char(keyemp), recurp, nombre, paterno, materno, coalesce(domemp, ' '), coalesce(colemp, ' '),
