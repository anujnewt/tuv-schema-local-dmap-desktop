create or replace procedure labprod."sp_nmlstcic"  (ws_nom_rep varchar,ws_ide_pcc varchar,wn_key_usu numeric,ws_key_men varchar,ws_hor_reg varchar,ws_lis_per varchar,ws_lis_pro varchar ) as $body$
declare
...
for c_nmlstcic in ( select mov_keypro, mov_keyper, mov_codimp, mov_keycon,  sum(mov_cantid ) alias5,  sum(mov_import ) alias6,  count(* ) alias7 from labprod.nmwkmovt
where mov_keypro in (
select ran_keypro from labprod.glwkrang
where ran_nomrep = ws_nom_rep
and ran_idepcc = ws_ide_pcc
...
begin select  count( distinct mov_keyemp ) alias1
into strict wn_tot_emp from labprod.nmwkmovt
