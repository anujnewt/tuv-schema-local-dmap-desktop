create or replace procedure labprod."sp_nmlstnoma"  (ws_nom_rep varchar,ws_des_rep varchar,ws_ide_pcc varchar,wn_key_usu numeric,ws_key_men varchar,ws_hor_reg varchar,wn_key_pro numeric,wn_tip_nom numeric,ws_key_per varchar ) as $body$
declare
...
wd_fec_rei timestamp(0);
wd_var_fec timestamp(0);
...
ws_opc_009:= null;
begin select pam_folini
into strict ws_opc_009 from labprod.glcopams
where pam_keypar = (
select pam_folini from labprod.glcopams
where pam_keypar='00'
and pam_cvesec='repnom')
...
into strict ws_nom_emp, ws_reg_rfc, wn_sal_dia, wn_sal_int, ws_key_dep, ws_key_pue, ws_tip_emp, ws_key_ims, ws_key_loc, ws_reg_ims,wd_fec_antig,wd_fec_baj,ws_rec_urp,wd_fec_ing,wd_fec_rei from labprod.nmcoempl
...
select 	ws_nom_rep,ws_ide_pcc,wn_key_usu, row_number() over () + j,ws_des_cor,emp_nomemp,'SAL.INT.',mov_keyemp,mov_keycon,con_descon,mov_import,'Proceso',mov_cantid,
...
-- and 	mov_import <> 0
order by  mov_keycon;/* dmap converted statement end */
