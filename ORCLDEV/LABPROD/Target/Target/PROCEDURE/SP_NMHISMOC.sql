create or replace procedure labprod."sp_nmhismoc"  (ws_nom_rep varchar,ws_ide_pcc varchar,wn_key_usu numeric,ws_key_men varchar,ws_hor_reg varchar,ws_etq_rep varchar ) as $body$
declare
...
for  c_descor in (select pro_keycia	from labprod.nmloproc
...
ws_etq_006:='DESCRIP';
begin select cam_descor	into strict ws_etq_006 from labprod.glcocamp	where cam_keytab='nmloconc'
...
ws_etq_008:='NOMBRE';
begin select cam_descor	into strict ws_etq_008 from labprod.glcocamp	where cam_keytab='nmcoempl'
...
call labprod.sp_glnewdsp ('nmlohism', 'his_keycon', ws_key_men,wn_dsp_002);
call labprod.sp_glnewdsp ('nmlohism', 'his_cantid', ws_key_men,wn_dsp_003);
...
begin select lis_deslis	into strict ws_var_xxx from labprod.glcolist where lis_keylis = ws_etq_rep;
exception
when no_data_found then
null;
end;
...
begin select con_descon	into strict ws_des_con from labprod.nmloconc	where con_keycon=ws_key_con;
exception
when no_data_found then
null;
end;
...
begin select pro_despro	into strict ws_des_pro from labprod.nmloproc	where pro_keypro=wn_key_pro;
exception
when no_data_found then
null;
end;
...
begin select emp_nomemp into strict ws_nom_emp from labprod.nmcoempl where emp_keyemp = wn_key_emp;
exception
when no_data_found then
null;
end;
