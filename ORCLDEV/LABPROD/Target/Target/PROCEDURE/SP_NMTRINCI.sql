create or replace procedure labprod."sp_nmtrinci"  ( wn_key_pro numeric, ws_per_fue varchar, wn_per_cer numeric, ws_nom_rep varchar, ws_ide_pcc varchar,wn_key_usu numeric, ws_per_des varchar,wn_rev_ers numeric, wn_key_inc numeric) as $body$
declare
...
if nullif(ws_con_fue::text, '') is not null or
nullif(ws_con_fue::text, '') is not null then
...
ws_con_aux := rtrim(ws_con_des::text);/* dmap converted statement end */
wn_num_sec := wn_num_sec + 1;
call sp_nmkeyinc (wn_key_inc,ws_con_aux,wn_cve_pro,wn_cve_nom,ws_per_des,wn_cve_emp,wn_num_sec,wn_inc_loc);
...
values (wn_cve_emp,ws_con_aux,wn_cve_pro,ws_per_des,ws_cve_dep,
...
wn_con_reg := wn_con_reg + 1;
wn_cve_emp := c_movtos.mov_keyemp;/* dmap converted statement start */
ws_cve_con := rtrim(c_movtos.mov_keycon::text);/* dmap converted statement end */
wn_cve_pro := c_movtos.mov_keypro;/* dmap converted statement start */
ws_cve_dep := rtrim(c_movtos.mov_keydep::text);/* dmap converted statement end *//* dmap converted statement start */
ws_cve_pue := rtrim(c_movtos.mov_keypue::text);/* dmap converted statement end */
