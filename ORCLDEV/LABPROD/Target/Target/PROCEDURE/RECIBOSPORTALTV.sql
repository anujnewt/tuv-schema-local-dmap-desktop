create or replace procedure labprod."recibosportaltv"  ( v_keybin varchar, keypro numeric, keyper varchar, keyemp numeric, cv_empdetails inout refcursor ) as $body$
declare
-- pgv moved types end
/*keypro  proceso
keyper  periodo */
...
-----------------------------------------------------
begin
insert into labprod.ps_tpw_msgxconcs(select v_keypro, v_keyper, con_keycon , 1 from labprod.nmloconc where con_keycon in ( '001' ));
end;
-----------------------------------------------------
-- 2 tot_isr
-----------------------------------------------------
begin
insert into labprod.ps_tpw_msgxconcs(select distinct v_keypro, v_keyper, con_keycon , 2
from labprod.glcopams
...
exception
when no_data_found then
insert into labprod.ps_tpw_msgxconcs(select v_keypro, v_keyper, con_keycon , 2 from labprod.nmloconc where con_keycon in ( '100','151','183','294','47A' )
);
end;
-----------------------------------------------------
-- 3 tot_per
...
begin
insert
into labprod.ps_tpw_msgxconcs(select distinct v_keypro, v_keyper, con_keycon , 3
from labprod.glcopams
join labprod.nmloconc on ( con_keycon  = pam_folfin )
where pam_keypar = v_keypar
...
exception
when no_data_found then
insert into labprod.ps_tpw_msgxconcs(select v_keypro, v_keyper, con_keycon, 3 from labprod.nmloconc  where con_keycon in ( '260','273','290' ));
end;
-----------------------------------------------------
-- 4 tot_ded
-----------------------------------------------------
insert
into labprod.ps_tpw_msgxconcs(select v_keypro, v_keyper, con_keycon, 4 from labprod.nmloconc
where con_keycon in ( '261','274','291','264' ));
-----------------------------------------------------
-- 5 imp_neto
-----------------------------------------------------
insert
into labprod.ps_tpw_msgxconcs(select v_keypro, v_keyper, con_keycon, 5
...
begin
insert
into labprod.ps_tpw_msgxconcs(select distinct v_keypro, v_keyper, con_keycon, 6
from labprod.glcopams
...
when no_data_found then
insert
into labprod.ps_tpw_msgxconcs( select v_keypro, v_keyper, con_keycon , 6 from labprod.nmloconc where con_keycon in ( 'D63','23D' ));
end;
-----------------------------------------------------
...
begin
insert
into labprod.ps_tpw_msgxconcs(select distinct v_keypro, v_keyper, con_keycon , 7
from labprod.glcopams
join labprod.nmloconc on ( con_keycon  = pam_folfin )
where pam_keypar = v_keypar
...
when no_data_found then
insert
into labprod.ps_tpw_msgxconcs(select v_keypro, v_keyper, con_keycon , 7
from labprod.nmloconc
...
begin
insert
into labprod.ps_tpw_msgxconcs(select distinct v_keypro, v_keyper, con_keycon, 8
from labprod.glcopams
join labprod.nmloconc on ( con_keycon  = pam_folfin )
where pam_keypar = v_keypar
...
when no_data_found then
insert
into labprod.ps_tpw_msgxconcs( select v_keypro, v_keyper, con_keycon , 8 from labprod.nmloconc where con_keycon in ( '28D' ));
end;/* dmap converted statement start */
-----------------------------------------------------
...
begin
insert
into labprod.ps_tpw_msgxconcs(select distinct v_keypro, v_keyper, con_keycon , 9
from labprod.nmloconc
...
when no_data_found then
insert
into labprod.ps_tpw_msgxconcs( select v_keypro, v_keyper, con_keycon , 9 from labprod.nmloconc where con_keycon in ( '423' ));
end;
-----------------------------------------------------
...
-----------------------------------------------------
insert
into labprod.ps_tpw_msgxconcs(select v_keypro, v_keyper, con_keycon, 10 from labprod.nmloconc
where con_keycon in ( '280' ,'H67'));
-----------------------------------------------------
-- 336 vales de fomento cultural
-- 11 vales de despensa
-----------------------------------------------------
insert
into labprod.ps_tpw_msgxconcs(select v_keypro, v_keyper, con_keycon, 11 from labprod.nmloconc
...
begin
insert
into labprod.ps_tpw_msgxconcs(select distinct v_keypro, v_keyper, pam_folfin, 13
from labprod.glcopams
where pam_keypar = v_keypar
...
begin
insert
into labprod.ps_tpw_msgxconcs(select distinct v_keypro, v_keyper, pam_folfin, 14
from labprod.glcopams
where pam_keypar = v_keypar
...
/* commit; */
v_nrchecksum := 0;
insert into labprod.tt_v_selempls values (v_keyemp);/* dmap converted statement start */
/* commit; */
if v_keynom = 25 then
...
sum(adeudo) adeudo
from
(   --esta es la diferencia del query entre la n????mina de aguinaldo y la nomina ordinaria
...
cast(his_codimp as numeric) his_codimp,
...
and his_keyemp = v_keyemp  -- 2043702
...
-- and ( v_nrchecksum = 0 or his_keyemp in (select keyemp from labprod.tt_v_selempls) )
and his_keyemp     = v_keyemp
group by his_keyemp
) hism_per on ( hism_per.his_keyemp = hem_tots.hem_keyemp )
where ( per_keypro                    = v_keypro
and per_keyper                        = v_keyper )
and his_keyemp = v_keyemp
--and ( v_nrchecksum = 0 or acu_keyemp in (select keyemp from labprod.tt_v_selempls) )
...
left outer join labprod.nmlopres on (pre_keyemp = his_keyemp and pre_keycon = his_keycon and pre_keypre = his_rowide)
where his_codimp in ( '01', '02' )
and con_nu1aux = (case when per_keynom in (1,26) then '1' else con_nu1aux end ) -- tipo de recibo 1 paranomina ordinaria
and per_keypro     = v_keypro
and per_keyper     = v_keyper
-- and ( v_nrchecksum = 0 or ( his_keyemp in (select keyemp from labprod.tt_v_selempls) ) )
and his_keyemp = v_keyemp  -- 2043702
...
and hem_keyemp = v_keyemp
group by hem_keyemp
) acums on ( acums.acu_keyemp = hem_tots.hem_keyemp )
left join(select his_keyemp ,
sum( case when msgs_tipocon = 1 and his_keynom not in (20,38) then his_cantid else 0 end) his_dias ,
sum( case msgs_tipocon when 2 then his_import else 0 end) tot_isr ,
sum( case msgs_tipocon when 3 then his_import else 0 end) tot_per ,
sum( case msgs_tipocon when 4 then his_import else 0 end) tot_ded ,
sum( case msgs_tipocon when 5 then his_import else 0 end) imp_neto ,
