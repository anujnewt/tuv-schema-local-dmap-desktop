CREATE OR REPLACE NONEDITIONABLE PROCEDURE "LABPROD"."SP_NMLSTCTT" (ws_nom_rep IN VARCHAR2,ws_ide_pcc IN VARCHAR2,wn_key_usu IN  NUMBER,ws_key_men IN VARCHAR2,ws_hor_reg IN VARCHAR2 ) IS
...
-- PGV moved types start
-- PGV moved types end
	    ws_pam_key VARCHAR2(6);
		ws_pam_cve VARCHAR2(6);
		-- ws_pam_nom VARCHAR2(50);
        ws_pam_nom VARCHAR2(200);
		...
		ws_tab_ant VARCHAR2(6);
		ws_des_lis VARCHAR2(50);
		ws_key_cam VARCHAR2(10);
		ws_des_etq VARCHAR2(10);
		ws_etq_001 VARCHAR2(200);
		ws_etq_002 VARCHAR2(200);
		ws_etq_003 VARCHAR2(200);
		ws_etq_004 VARCHAR2(200);
		ws_etq_005 VARCHAR2(200);
		ws_etq_006 VARCHAR2(200);
		ws_etq_007 VARCHAR2(200);
...
BEGIN
	SELECT  COUNT(*) alias1
	INTO wn_tot_reg FROM LABPROD.glcopams
	WHERE pam_keypar IN (
	SELECT pam_cvesec FROM LABPROD.glcopams
	WHERE pam_keypar='TT')
	AND pam_keypar IN (
	...
	wn_dsp_004:=0;
	wn_dsp_005:=0;
	LABPROD.sp_glNewDsp('glcopams', 'pam_keypar', ws_key_men,wn_dsp_001);
	LABPROD.sp_glNewDsp('glcopams', 'pam_cvesec', ws_key_men,wn_dsp_002);
	LABPROD.sp_glNewDsp('glcopams', 'pam_nompar', ws_key_men,wn_dsp_003);
	LABPROD.sp_glNewDsp('glcopams', 'pam_folini', ws_key_men,wn_dsp_004);
	LABPROD.sp_glNewDsp('glcopams', 'pam_folfin', ws_key_men,wn_dsp_005);
	ws_tab_ant:='@@@';
	wn_num_reg:=0;
	wn_pct_act:=1;
	wn_pct_reg:=(wn_tot_reg/10.0);
	ws_etq_006:='Nada';
	FOR c_nmlstcic IN ( SELECT pam_keypar, pam_cvesec, pam_nompar, pam_folini, pam_folfin FROM LABPROD.glcopams
	WHERE pam_keypar IN (
	SELECT pam_cvesec FROM LABPROD.glcopams
	WHERE pam_keypar='TT')
	AND pam_keypar IN (
	...
	ORDER BY pam_keypar,pam_cvesec ) LOOP
		ws_pam_key :=c_nmlstcic.pam_keypar;
		ws_pam_cve :=c_nmlstcic.pam_cvesec;
		ws_pam_nom :=c_nmlstcic.pam_nompar;
		ws_pam_ini :=c_nmlstcic.pam_folini;
		ws_pam_fin :=c_nmlstcic.pam_folfin;
		...
		ws_etq_001:='No Existe Nombre ..';
		ELSE
		IF (ws_tab_ant!=ws_pam_key ) THEN
		ws_etq_007:='No Existe Nombre ..';
		SELECT pam_nompar
		INTO ws_etq_007 FROM LABPROD.glcopams
		WHERE pam_keypar ='TT' AND pam_cvesec=ws_pam_key;
		...
		ws_pam_fin:=NULL;
		END IF;
		INSERT INTO LABPROD.glwkcrys( cry_nomrep,cry_idepcc,cry_keyusu,cry_chr001,cry_chr002,cry_chr014,cry_chr004,cry_chr005,cry_chr006,cry_chr007,cry_chr008,cry_chr009,cry_chr010,cry_chr011,cry_chr012,cry_chr013,cry_dat001,cry_chr003)
		VALUES(ws_nom_rep,ws_ide_pcc,wn_key_usu,ws_des_cor,ws_pam_key,ws_pam_cve,ws_pam_nom,ws_pam_ini,ws_pam_fin,ws_etq_007,ws_etq_001,ws_etq_002,ws_etq_003,ws_etq_004,ws_etq_005,ws_hor_act,ws_dia_act,ws_des_lis); COMMIT;
	END LOOP;
	LABPROD.sp_glfechor(ws_dia_act, ws_hor_act);
	UPDATE LABPROD.glcoresu
	SET res_numreg=wn_num_reg,res_fecfin=ws_dia_act,res_horfin=ws_hor_act,res_status='T'
	WHERE res_idepro=ws_nom_rep
	AND res_idepcc=ws_ide_pcc
	AND res_keyusu=wn_key_usu
	AND res_fecini=ws_dia_act
