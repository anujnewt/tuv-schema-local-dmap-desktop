CREATE OR REPLACE NONEDITIONABLE PROCEDURE "LABPROD"."SP_SIPS_ORAC_EMPL2" 
...
--Igneos.I sin consultas a nmcoempl (se trae el proceso)
vn_keypro in number)
IS
...
IF vn_keypro = 81 OR vn_keypro = 17 OR vn_keypro = 18 OR vn_keypro = 3  OR vn_keypro = 77 OR vn_keypro = 8  OR vn_keypro = 66 THEN
  IF tipo = 'IN' THEN
    ...
    vn_apelli := trim(sp_delimitador(vp_nomemp,'/',1)) || '/' || trim(sp_delimitador(vp_nomemp,'/',2));
    vn_nomemp := trim(sp_delimitador(vp_nomemp,'/',3));
    if vp_cvesex = '1' then
      vn_cvesex := 'M';
    else
      vn_cvesex := 'F';
