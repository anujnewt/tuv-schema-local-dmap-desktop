CREATE OR REPLACE NONEDITIONABLE PROCEDURE "LABPROD"."SP_NMLSTPU2" (ws_nom_rep IN VARCHAR2,   ws_ide_pcc IN VARCHAR2,
...
BEGIN
    ...
    COMMIT;
    -- Cursor principal.
    FOR c_nmlstpue IN (SELECT pue_keypue, pue_despue, rtrim(pue_grppue) pue_grppue,
				rtrim(pue_subgrp) pue_subgrp, tab_keysue,
	...
	ws_key_sue := c_nmlstpue.tab_keysue;
	...
	IF ws_key_sue != ws_sue_ant THEN
		...
		ws_des_sue := SUBSTR (c_dessue.sue_dessue, 1, 40);
		ws_etq_min := SUBSTR (c_dessue.sue_etqmin, 1,  8);
		...
		ws_etq_max := SUBSTR (c_dessue.sue_etqmax, 1,  8);
	    END LOOP;
	    ws_sue_ant := SUBSTR (ws_key_sue, 1, 4);
	END IF;
	IF (c_nmlstpue.pue_grppue IS NOT NULL) AND (c_nmlstpue.pue_grppue != ' ') THEN
	...
	ELSE
	    ...
	    ws_etq_002 := ' ';
	END IF;
	IF (c_nmlstpue.pue_subgrp IS NOT NULL) AND (c_nmlstpue.pue_subgrp != ' ') THEN
