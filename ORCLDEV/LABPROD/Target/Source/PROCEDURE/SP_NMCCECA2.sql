CREATE OR REPLACE NONEDITIONABLE PROCEDURE "LABPROD"."SP_NMCCECA2" (ws_nom_rep IN VARCHAR2,ws_ide_pcc IN VARCHAR2,wn_key_usu IN  NUMBER,ws_key_men IN VARCHAR2,ws_hor_reg  IN VARCHAR2,ws_lis_per  IN VARCHAR2,ws_lis_pro IN VARCHAR2,ws_lis_dep IN VARCHAR2 ) IS
...
EXCEPTION
 WHEN NO_DATA_FOUND THEN
  ...
  ws_etq_001:='----------';
	...
	wn_pri_mer:=0;
	wn_tot_tra:=0;
	FOR c_nmlstcic IN ( SELECT mov_keypro, mov_keyper, mov_ca2aux, mov_codimp, mov_keycon,  SUM(mov_cantid ) alias6,  SUM(mov_import ) alias7,  COUNT(* ) alias8
	FROM LABPROD.nmwkmovt, LABPROD.glwkrang
	WHERE mov_keypro = ran_keypro
	AND mov_keyper = ran_keyper
	AND mov_ca2aux = ran_keydep
	...
	GROUP BY mov_ca2aux, mov_keypro, mov_keyper,  mov_codimp, mov_keycon
	ORDER BY mov_ca2aux, mov_keypro, mov_keyper,  mov_codimp, mov_keycon ) LOOP
		wn_mov_pro :=c_nmlstcic.mov_keypro;
			ws_mov_per :=c_nmlstcic.mov_keyper;
			ws_mov_dep :=c_nmlstcic.mov_ca2aux;
...
EXCEPTION
 WHEN NO_DATA_FOUND THEN
  ...
  END IF;
	...
	LABPROD.sp_nomdes(ws_mov_dep, 2,ws_des_dep);
