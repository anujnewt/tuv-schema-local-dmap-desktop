CREATE OR REPLACE NONEDITIONABLE PACKAGE "LABPROD"."TVCONTAB" IS
	TYPE argumentos IS RECORD ( idepro LABPROD.glcoargu.arg_idepro%TYPE, idepcc LABPROD.glcoargu.arg_idepcc%TYPE,
								keyusu LABPROD.glcoargu.arg_keyusu%TYPE, fecini LABPROD.glcoargu.arg_fecini%TYPE,
								horini LABPROD.glcoargu.arg_horini%TYPE);
	TYPE partefija  IS RECORD ( keyemp LABPROD.nmlohism.his_keyemp%TYPE, keypro LABPROD.nmlohism.his_keypro%TYPE,
								keycon LABPROD.nmlohism.his_keycon%TYPE, codimp LABPROD.nmlohism.his_codimp%TYPE,
								descon LABPROD.nmloconc.con_descon%TYPE, keycia LABPROD.nmloproc.pro_keycia%TYPE,
								keypol LABPROD.nmloperi.per_keypol%TYPE, cveban LABPROD.nmcoempl.emp_cveban%TYPE,
								forpag LABPROD.nmcoempl.emp_forpag%TYPE, keyben LABPROD.nmlohism.his_keyben%TYPE,
								comfam LABPROD.nmlohism.his_comfam%TYPE, fecmov LABPROD.nmlohism.his_fecmov%TYPE);
	---------------------------------
	-- procedimientos
    PROCEDURE SP_POLIZA (idepro IN LABPROD.glcoargu.arg_idepro%TYPE, idepcc IN LABPROD.glcoargu.arg_idepcc%TYPE,
						 keyusu IN LABPROD.glcoargu.arg_keyusu%TYPE, fecini IN LABPROD.glcoargu.arg_fecini%TYPE,
						 horini IN LABPROD.glcoargu.arg_horini%TYPE);
    PROCEDURE SP_CONTABLE (proceso IN LABPROD.nmlohism.his_keypro%TYPE, periodo IN LABPROD.nmlohism.his_keyper%TYPE);
    PROCEDURE SP_INSERTA (datospf IN partefija, referencia IN LABPROD.nmcodeps.dep_refcon%TYPE,
						  ctaconcepto IN LABPROD.nmloconc.con_ctaref%TYPE,seietu IN LABPROD.nmloconc.con_porcen%TYPE,
						  impcar IN LABPROD.nmlohism.his_import%TYPE, impabo IN LABPROD.nmlohism.his_import%TYPE,
						  tipo IN LABPROD.tvwkpoli.pol_tipo%TYPE);
    PROCEDURE SP_FINIQUITO (empleado IN LABPROD.nmwkmovt.mov_keyemp%TYPE, periodo IN LABPROD.nmwkmovt.mov_keyper%TYPE,
							proceso IN LABPROD.nmcoempl.emp_keypro%TYPE);
    PROCEDURE SP_RECLASIFICA (proceso IN LABPROD.nmlohism.his_keypro%TYPE, periodo IN LABPROD.nmlohism.his_keyper%TYPE,
							  keypol IN LABPROD.tvwkpoli.pol_keypol%TYPE);
	--------------------------------
    --funciones
END TVCONTAB;
/
CREATE OR REPLACE NONEDITIONABLE PACKAGE BODY "LABPROD"."TVCONTAB" IS
/* -- Variables Globales-------------------------------------------- */
    poliza 	partefija;  -- Campos que no var??an en la p??liza para un registro espec??fico
    args 		argumentos;  -- valores de la glcoargu
    proceso	LABPROD.nmlohism.his_keypro%TYPE;  -- proceso guardado en la glcoargu
    periodo LABPROD.nmlohism.his_keyper%TYPE;  -- periodo guardado en la glcoargu
    nomina 	LABPROD.nmlohism.his_keynom%TYPE;   -- Tipo de Nomina
    -- -----------------------------------------------------------------------------------------------------------------
    -- ELJM OPCIS PARA REGISTRO DUPLICADO
   	DR_ciaori LABPROD.TVWKPOLI.POL_CIA%TYPE;  	-- CIA ORIGEN PARA APLICAR DOBLE REGISTRO		'019'
   	DR_ciades LABPROD.TVWKPOLI.POL_CIA%TYPE;  	-- CIA DESTINO PARA APLICAR DOBLE REGISTRO	'522'
    DR_concep LABPROD.NMLOHISM.HIS_KEYCON%TYPE; -- CVE CONCEPTO DE COMISIONES					    	'M39'
    DR_cta   	LABPROD.TVWKPOLI.POL_CTA%TYPE;		-- CTA PARA AFECTACION DOBLE REGISTRO       '126'
    DR_scta   LABPROD.TVWKPOLI.POL_SCTA%TYPE;  	-- SCTA PARA AFECTACION	DOBLE REGISTRO 			'012002'
    -- -----------------------------------------------------------------------------------------------------------------
/* -- Constantes -------------------------------------------- */
    argpro  	LABPROD.glcoargu.arg_keycam%TYPE := 'KEYPRO'; -- clave de campo en la glcoargu para proceso
    argper  	LABPROD.glcoargu.arg_keycam%TYPE := 'KEYPER'; -- clave de campo en la glcoargu para periodo
    madre   	LABPROD.tvwkpoli.pol_tipo%TYPE := 1; -- clave para la cuenta madre
    hija    	LABPROD.tvwkpoli.pol_tipo%TYPE := 2; -- clave para la cuenta hija
    regs    INTEGER := 10; -- numero de registros antes de actualizar en la glcoresu
    PROCEDURE SP_RECLASIFICA2(proceso IN LABPROD.nmlohism.his_keypro%TYPE, periodo IN LABPROD.nmlohism.his_keyper%TYPE,keypol IN LABPROD.tvwkpoli.pol_keypol%TYPE);
--PROCEDIMIENTOS /* -------------------------------------------------------------------------- */
  PROCEDURE SP_POLIZA (idepro IN LABPROD.glcoargu.arg_idepro%TYPE, idepcc IN LABPROD.glcoargu.arg_idepcc%TYPE, keyusu IN LABPROD.glcoargu.arg_keyusu%TYPE,
                        fecini IN LABPROD.glcoargu.arg_fecini%TYPE, horini IN LABPROD.glcoargu.arg_horini%TYPE) IS
   BEGIN
       args.idepro := idepro;  args.idepcc := idepcc;  args.keyusu := keyusu;  args.fecini := fecini;  args.horini := horini;
       SELECT arg_pvalor INTO proceso FROM LABPROD.glcoargu
       ...
       SELECT arg_pvalor INTO periodo FROM LABPROD.glcoargu
       WHERE arg_idepro = idepro AND arg_idepcc = idepcc AND arg_keyusu = keyusu
       AND arg_fecini = fecini AND arg_horini = horini AND arg_keycam = argper;
       SP_CONTABLE(proceso, periodo);
   END SP_POLIZA;
/* -------------------------------------------------------------------------- */
  PROCEDURE SP_CONTABLE (proceso IN LABPROD.nmlohism.his_keypro%TYPE, periodo IN LABPROD.nmlohism.his_keyper%TYPE) IS
      total     INTEGER;
      i         INTEGER;
      CveVersion   INTEGER;
      CvePoliza LABPROD.nmloperi.per_keypol%Type;
      CveVersionProc INTEGER;
  BEGIN
  -- marcar el periodo per_selper = P para procesar
  update LABPROD.nmloperi set per_persel = 'P' where per_keyper = periodo and per_keypro = proceso;
  COMMIT;
  ...
  -- -------------------------------------------------------------------------------------------------------------------
  -- borra p??liza
  delete from LABPROD.tvwkpoli where pol_keypol = CvePoliza;
  COMMIT;
  -- cuenta
  SELECT count(*) into total from LABPROD.nmlohism
      inner join LABPROD.nmcodeps on (his_keydep = dep_keydep)
      inner join LABPROD.nmloperi on (his_keyper = per_keyper and his_keypro = per_keypro)
      inner join LABPROD.nmcoempl on (his_keyemp = emp_keyemp)
      inner join labprod.nmloproc ON (his_keypro = pro_keypro)
      INNER JOIN LABPROD.tvloverc Ver0 ON (Ver0.ver_keyver = pro_vercon and his_keycon = Ver0.ver_keycon)
      LEFT JOIN LABPROD.tvloverc Ver ON (Ver.ver_keyver = CveVersion and his_keycon = Ver.ver_keycon AND Ver.ver_keyver <>0)
      where his_keyper = periodo
      and his_keypro = proceso
      and (length(dep_refcon) = 52 or length(dep_refcon) = 26);
  IF total = 0 THEN
   update LABPROD.nmloperi set per_persel = '' where per_keyper = periodo and per_keypro = proceso;
   UPDATE LABPROD.glcoresu SET res_status = 'T',res_totreg = 0,res_numreg = 0
  ...
  -- inicializa registro en glcoresu (el registro ya fue insertado desde el programa del cliente
    UPDATE LABPROD.glcoresu SET res_numreg = i, res_totreg = total
  ...
  -- procesa
    ...
    LOOP
       ...
       -- inserta registro tipo 2
        ...
        IF (i MOD regs = 0) THEN
            UPDATE LABPROD.glcoresu SET res_numreg = i
    ...
    UPDATE LABPROD.glcoresu SET res_numreg = total, res_status = 'T'
    WHERE res_idepro = args.idepro AND res_idepcc = args.idepcc
    AND res_keyusu = args.keyusu AND res_fecini = args.fecini AND res_horini = args.horini;
  ---Fin Contable
  END SP_CONTABLE;
  PROCEDURE SP_INSERTA (datospf IN partefija, 										referencia IN LABPROD.nmcodeps.dep_refcon%TYPE,
  											ctaconcepto IN LABPROD.nmloconc.con_ctaref%TYPE,	seietu IN LABPROD.nmloconc.con_porcen%TYPE,
  											impcar IN LABPROD.nmlohism.his_import%TYPE, 			impabo IN LABPROD.nmlohism.his_import%TYPE,
  											tipo IN LABPROD.tvwkpoli.pol_tipo%TYPE) IS
      cia 			VARCHAR2(3);
      neg 			VARCHAR2(2);
      cuenta 		VARCHAR2(3);
      subcta 		VARCHAR2(6);
  ...
  BEGIN
        ...
        IF tipo = madre THEN
            ...
            IF cia <> '000' AND SUBCTA <> '000000' THEN
              INSERT INTO LABPROD.tvwkpoli VALUES (datospf.keyemp, datospf.keypro,
         ...
         ELSE  --hija
            ...
            END IF;
            IF cia <> '000' AND SUBCTA <> '000000' THEN
              INSERT INTO LABPROD.tvwkpoli VALUES (datospf.keyemp, datospf.keypro, datospf.keycon,
  ...
  commit;
  END SP_INSERTA;
 PROCEDURE SP_FINIQUITO (empleado IN LABPROD.nmwkmovt.mov_keyemp%TYPE,
						 periodo IN LABPROD.nmwkmovt.mov_keyper%TYPE,
						 proceso IN LABPROD.nmcoempl.emp_keypro%TYPE) IS
  ...
  END SP_FINIQUITO;
--FUNCIONES
  PROCEDURE SP_RECLASIFICA (proceso IN LABPROD.nmlohism.his_keypro%TYPE, periodo IN LABPROD.nmlohism.his_keyper%TYPE,keypol IN LABPROD.tvwkpoli.pol_keypol%TYPE) IS
    ws_cia VARCHAR2(3);
  BEGIN
    INSERT INTO LABPROD.TVWKPOLI
    ...
    END IF;
  END SP_RECLASIFICA;
  PROCEDURE SP_RECLASIFICA2 (proceso IN LABPROD.nmlohism.his_keypro%TYPE, periodo IN LABPROD.nmlohism.his_keyper%TYPE,keypol IN LABPROD.tvwkpoli.pol_keypol%TYPE) IS
    ws_cia VARCHAR2(3);
  BEGIN
