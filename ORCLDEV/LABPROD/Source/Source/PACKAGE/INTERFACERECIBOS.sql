CREATE OR REPLACE NONEDITIONABLE PACKAGE "LABPROD"."INTERFACERECIBOS" IS
   ---------------------------------
...
/
CREATE OR REPLACE NONEDITIONABLE PACKAGE BODY "LABPROD"."INTERFACERECIBOS" IS
    ...
    BEGIN
        MERGE INTO LABPROD.CFDI2ESTATUSRECIBOS
        USING
        ...
        ON
            (LABPROD.CFDI2ESTATUSRECIBOS.IDCOMPROBANTEEMP = INSERTARESTATUSRECIBO.IDCOMPROBANTEEMP)
        WHEN MATCHED THEN
    ...
    BEGIN
        INSERT INTO LABPROD.RECIBOS_INTERFACE (EJE_FECEJE,EJE_MENSAJ,EJE_ERROR)
    ...
    BEGIN
        UPDATE LABPROD.RECIBOS_INTERFACE SET
            EJE_FECEJE = SYSDATE,
    ...
    BEGIN
        INSERT INTO LABPROD.RECIBOS_GENERACION (EJE_FECEJE,EJE_MENSAJ,EJE_ERROR,EJE_KEYPRO,EJE_KEYPER)
    ...
    BEGIN
        UPDATE LABPROD.RECIBOS_GENERACION SET
            EJE_FECEJE = SYSDATE,
      ...
      DECLARE
        ...
        ------------------ Clave de OPCIS  ------------------
            ...
            BEGIN
                ...
                (SELECT v_KeyPro, v_KeyPer, CON_KEYCON , 1 FROM NMLOCONC WHERE CON_KEYCON IN ( '001' ));
            END;
            -----------------------------------------------------
            -- 2 TOT_ISR
            -----------------------------------------------------
            ...
            EXCEPTION
              WHEN NO_DATA_FOUND THEN
                INSERT INTO labprod.PS_TPW_MSGXCONCS
                  (SELECT v_KeyPro, v_KeyPer, CON_KEYCON , 2 FROM NMLOCONC WHERE CON_KEYCON IN ( '100','151','183','294','47A' )
                  );
            END;
            -----------------------------------------------------
            -- 3 TOT_PER
            ...
            EXCEPTION
              WHEN NO_DATA_FOUND THEN
              INSERT INTO labprod.PS_TPW_MSGXCONCS
              (SELECT v_KeyPro, v_KeyPer, CON_KEYCON, 3 FROM NMLOCONC  WHERE CON_KEYCON IN ( '260','273','290' ));
            END;
            -----------------------------------------------------
            -- 4 TOT_DED
            -----------------------------------------------------
