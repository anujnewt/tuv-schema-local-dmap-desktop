CREATE OR REPLACE NONEDITIONABLE PACKAGE "LABPROD"."CALCULOISN" AS
...
END CALCULOISN;
/
CREATE OR REPLACE NONEDITIONABLE PACKAGE BODY "LABPROD"."CALCULOISN" AS
  ...
  BEGIN
    ...
    WHERE per_keypro IN (SELECT ran_keypro FROM LABPROD.glwkrang WHERE ran_nomrep = ws_nom_rep)
    AND per_keyper = ws_key_per
    AND ROWNUM = 1;
    DELETE FROM LABPROD.NMLOHEMP
    WHERE hem_keypro IN (SELECT ran_keypro FROM LABPROD.GLWKRANG WHERE ran_nomrep = ws_nom_rep)
      AND hem_keyper = ws_key_per;
    INSERT INTO LABPROD.NMLOHEMP (hem_keypro,hem_keyper,hem_keyemp,hem_keydep,hem_ca1aux,hem_diades,hem_keypue,hem_keycen,hem_tipemp,hem_keyloc,hem_keyims,hem_fecaum,hem_salmes)
    SELECT DISTINCT hem_keypro, ws_key_per,hem_keyemp,hem_keydep,hem_ca1aux,LABPROD.EDADXFECHA(hem_regrfc,wd_fec_fin),emp_keypue,emp_keycen,emp_tipemp,emp_keyloc,emp_keyims,emp_fecaum,emp_salmes
    FROM LABPROD.nmlohemp
    INNER JOIN LABPROD.nmloperi ON hem_keypro = per_keypro and hem_keyper = per_keyper
    INNER JOIN LABPROD.nmlohism ON his_keypro = hem_keypro AND his_keyper = hem_keyper AND his_keyemp = hem_keyemp AND his_keycon IN ('311','268','C25','658','627','281','334')  --Este join es para evitar que se inserten registros sin movimientos
    LEFT JOIN LABPROD.nmcoempl ON hem_keyemp = emp_keyemp
    WHERE per_keypro in (SELECT ran_keypro FROM LABPROD.glwkrang WHERE ran_nomrep = ws_nom_rep)
  ...
  BEGIN
    ...
    SELECT hem_keyemp,  --mov_keyemp
           ...
           0   --mov_comfam
    FROM labprod.nmloperi
    INNER JOIN labprod.nmlohemp ON hem_keypro = per_keypro AND hem_keyper = per_keyper
    INNER JOIN labprod.nmlohism ON hem_keyemp = his_keyemp AND per_keypro = his_keypro AND per_keyper = his_keyper
    WHERE per_anioa1 = TO_CHAR(wn_anio)
      AND per_nummes = wn_mes
      AND per_keypro IN (SELECT ran_keypro FROM LABPROD.glwkrang WHERE ran_nomrep = ws_nom_rep)
      AND per_keynom IN (1,3,6,14,15,16,20,23,25,26,30,32,38,62,31)
      AND his_keycon IN (SELECT DISTINCT con_keycon FROM LABPROD.IsnConf WHERE con_anio = wn_anio)
      AND his_codacu <> 'NO'
  ...
  BEGIN
    ...
    SELECT hem_keyemp,  --mov_keyemp
           ...
           0   --mov_comfam
    FROM LABPROD.nmlohemp
    INNER JOIN LABPROD.nmcoinci ON hem_keyemp = inc_keyemp AND hem_keypro = inc_keypro AND hem_keyper = inc_keyper AND inc_keycon = 'S07'
    WHERE hem_keypro IN (SELECT ran_keypro FROM LABPROD.glwkrang WHERE ran_nomrep = ws_nom_rep)
      AND hem_keyper = ws_key_per
      AND hem_ca1aux IN (SELECT pam_cvesec FROM LABPROD.glcopams WHERE pam_keypar = 'IEST' AND pam_folini IN ('15','03'))
  ...
  BEGIN
    ...
    UPDATE IsnTotales
    SET isn_porcen = LABPROD.CALCULOISN.PORCENTAJE(isn_keyent, isn_porcen,isn_keytab,isn_base),
        isn_poradi = LABPROD.CALCULOISN.PORCENTAJE_ADICIONAL(isn_keyent, isn_poradi,isn_keytab,isn_base,isn_keycia, wn_anio),
        isn_cuofij = LABPROD.CALCULOISN.CUOTAFIJA(isn_keyent, isn_porcen,isn_keytab,isn_base)
    WHERE isn_keycia IN (SELECT pro_keycia
                         FROM LABPROD.glwkrang
                         INNER JOIN LABPROD.nmloproc ON pro_keypro = ran_keypro
                         WHERE ran_nomrep = ws_nom_rep)
      ...
      AND isn_mes = wn_mes;
    UPDATE IsnTotales
    SET isn_impisn = LABPROD.CALCULOISN.IMPORTEISN(isn_keyent,isn_base, isn_porcen, isn_poradi, isn_keytab,isn_cuofij, isn_totreg)
    WHERE isn_keycia IN (SELECT pro_keycia
                         FROM LABPROD.glwkrang
                         INNER JOIN LABPROD.nmloproc ON pro_keypro = ran_keypro
                         WHERE ran_nomrep = ws_nom_rep)
      ...
      AND isn_mes = wn_mes;
    UPDATE IsnTotales
    SET isn_impadi = LABPROD.CALCULOISN.IMPORTEADI(isn_keyent,isn_base, isn_porcen, isn_poradi, isn_keytab, isn_impisn)
    WHERE isn_keycia IN (SELECT pro_keycia
                         FROM LABPROD.glwkrang
                         INNER JOIN LABPROD.nmloproc ON pro_keypro = ran_keypro
                         WHERE ran_nomrep = ws_nom_rep)
    ...
    CASE ws_key_ent
      ...
      ELSE
        BEGIN
          ...
          wn_imp_adi := LABPROD.CALCULOISN.IMPORTEADI(ws_key_ent,wn_base, wn_por_cen, wn_por_adi, ws_key_tab, wn_imp_isn);
  ...
  BEGIN
    UPDATE LABPROD.glcoresu SET res_status = ws_sta_tus,res_numreg = wn_num_reg,res_totreg = wn_tot_reg, res_deserr=ws_men_saj
  ...
  BEGIN
    CASE
      WHEN ws_key_ent = '12' THEN   --GUERRERO
        SELECT CASE WHEN pam_folini IS NULL THEN ENT_PORADI ELSE NVL(LABPROD.SAFE_TO_NUMBER(pam_folini),0) END INTO wn_result
  ...
  BEGIN
    ...
    WHERE hem_keypro IN (SELECT ran_keypro FROM LABPROD.glwkrang WHERE ran_nomrep = ws_nom_rep)
    AND hem_keyper = ws_key_per
    AND hem_ca1aux IN (SELECT pam_cvesec FROM LABPROD.glcopams WHERE pam_keypar  ='IEST' AND pam_folini = '22');
    SELECT SUM(mov_import) INTO wn_tot_bas
    FROM LABPROD.nmwkmovt
    WHERE mov_keypro IN (SELECT ran_keypro FROM LABPROD.glwkrang WHERE ran_nomrep = ws_nom_rep)
    AND mov_keyper = ws_key_per
    AND mov_ca1aux IN (SELECT pam_cvesec FROM LABPROD.glcopams WHERE pam_keypar  ='IEST' AND pam_folini = '22')
    AND mov_keycon = 'B22';
  ...
  BEGIN
    ...
    WHERE hem_keypro IN (SELECT ran_keypro FROM LABPROD.glwkrang WHERE ran_nomrep = ws_nom_rep)
    AND hem_keyper = ws_key_per
    AND hem_ca1aux IN (SELECT pam_cvesec FROM LABPROD.glcopams WHERE pam_keypar  ='IEST' AND pam_folini = '26');
    IF wn_tot_emp  >= 1 AND wn_tot_emp <= 5 THEN
      INSERT INTO LABPROD.nmwkmovt (MOV_KEYEMP,MOV_KEYCON,MOV_KEYNOM,MOV_KEYDEP,MOV_KEYPUE,MOV_CANTID,MOV_IMPORT,MOV_FECMOV,MOV_KEYPER,MOV_KEYPRO,MOV_KEYFOR,MOV_CODIMP,MOV_CODACU,MOV_ROWIDE,MOV_CA1AUX,MOV_CA2AUX,MOV_UNIOPE,MOV_KEYPLZ,MOV_TIPPLZ,MOV_KEYBEN,MOV_COMFAM)
        ...
        WHERE mov_keypro IN (SELECT ran_keypro FROM LABPROD.glwkrang WHERE ran_nomrep = ws_nom_rep)
        AND mov_keyper = ws_key_per
        AND mov_ca1aux IN (SELECT pam_cvesec FROM LABPROD.glcopams WHERE pam_keypar  ='IEST' AND pam_folini = '26')
        AND mov_keycon = 'B26';
    END IF;
  ...
  BEGIN
    ...
    INNER JOIN LABPROD.glcopams IEST ON pam_keypar = 'IEST' AND pam_cvesec = hem_ca1aux AND pam_folini = '28'
    WHERE hem_keypro IN (SELECT ran_keypro FROM LABPROD.glwkrang WHERE ran_nomrep = ws_nom_rep)
    AND hem_keyper = ws_key_per
    ...
    SELECT hem_keyemp,  --mov_keyemp
           ...
           NULL, --mov_keypue
           0, --mov_cantid
           LABPROD.EXENTO_INDEMNIZACION(hem_fecaux,hem_fecing,SUM(mov_import),wn_uma),  --mov_import
           per_fecfin,   --mov_fecmov
           hem_keyper,   --mov_keyper
           hem_keypro,   --mov_keypro
           'ISNE',  --mov_keyfor
    ...
    INNER JOIN LABPROD.glcopams IEST ON pam_keypar = 'IEST' AND pam_cvesec = hem_ca1aux AND pam_folini = '28'
    --INNER JOIN LABPROD.nmcoempl ON hem_keyemp = emp_keyemp
    WHERE hem_keypro IN (SELECT ran_keypro FROM LABPROD.glwkrang WHERE ran_nomrep = ws_nom_rep)
    AND hem_keyper = ws_key_per
  ...
  BEGIN
    ...
    INNER JOIN LABPROD.glcopams IEST ON pam_keypar = 'IEST' AND pam_cvesec = hem_ca1aux AND pam_folini = ws_key_ent
    WHERE hem_keypro IN (SELECT ran_keypro FROM LABPROD.glwkrang WHERE ran_nomrep = ws_nom_rep)
    AND hem_keyper = ws_key_per
