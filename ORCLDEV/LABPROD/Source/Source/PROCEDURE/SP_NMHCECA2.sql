CREATE OR REPLACE NONEDITIONABLE PROCEDURE "LABPROD"."SP_NMHCECA2" (ws_nom_rep IN VARCHAR2,ws_ide_pcc IN VARCHAR2,wn_key_usu IN  NUMBER,ws_key_men IN VARCHAR2,ws_hor_reg  IN VARCHAR2,ws_lis_per  IN VARCHAR2,ws_lis_pro IN VARCHAR2,ws_lis_dep IN VARCHAR2 ) IS
...
EXCEPTION
 WHEN NO_DATA_FOUND THEN
  ...
  ws_etq_001:='----------';
	...
	END IF;
	END LOOP;
	FOR c_nmlsthis IN ( SELECT his_keypro, his_keyper, his_ca2aux, his_codimp, his_keycon,  SUM(his_cantid ) alias6,  SUM(his_import ) alias7,  COUNT(* ) alias8
	...
	GROUP BY his_ca2aux, his_keypro, his_keyper, his_codimp, his_keycon
	ORDER BY his_ca2aux, his_keypro, his_keyper, his_codimp, his_keycon ) LOOP
		wn_his_pro :=c_nmlsthis.his_keypro;
			ws_his_per :=c_nmlsthis.his_keyper;
			ws_his_dep :=c_nmlsthis.his_ca2aux;
	...
	INTO wn_his_emp FROM LABPROD.nmlohism
	WHERE his_keyper IN (
	SELECT ran_keyper FROM LABPROD.glwkrang
	WHERE ran_nomrep = ws_nom_rep
	AND ran_idepcc = ws_ide_pcc
	...
	AND ran_keyusu = wn_key_usu
	AND ran_keypro IS NOT NULL )
	AND his_ca2aux IS NULL;
EXCEPTION
 WHEN NO_DATA_FOUND THEN
  NULL;
  END;
  BEGIN SELECT  COUNT( DISTINCT his_keyemp ) alias1
	INTO wn_sum_pro FROM LABPROD.nmlohism
	WHERE his_keyper IN (
	SELECT ran_keyper FROM LABPROD.glwkrang
	WHERE ran_nomrep = ws_nom_rep
	AND ran_idepcc = ws_ide_pcc
...
EXCEPTION
 WHEN NO_DATA_FOUND THEN
  ...
  END;
  BEGIN SELECT  COUNT( DISTINCT his_keyemp ) alias1
	INTO wn_sum_per FROM LABPROD.nmlohism
...
EXCEPTION
 WHEN NO_DATA_FOUND THEN
  ...
  END IF;
	...
	sp_nomdes(ws_his_dep, 2,ws_des_dep);
	...
	INTO ws_des_pro, ws_his_cia FROM LABPROD.nmloproc
...
EXCEPTION
 WHEN NO_DATA_FOUND THEN
  ...
  ws_ant_con:=ws_his_con;
	...
	INTO wd_fec_ini, wd_fec_fin, wn_his_nom FROM LABPROD.nmloperi
