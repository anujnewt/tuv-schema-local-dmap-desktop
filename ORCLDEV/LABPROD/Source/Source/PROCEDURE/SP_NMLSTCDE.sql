CREATE OR REPLACE NONEDITIONABLE PROCEDURE "LABPROD"."SP_NMLSTCDE" (ws_nom_rep IN VARCHAR2,ws_ide_pcc IN VARCHAR2,wn_key_usu IN  NUMBER,ws_key_men IN VARCHAR2,ws_hor_reg  IN VARCHAR2,ws_lis_per  IN VARCHAR2,ws_lis_pro IN VARCHAR2,ws_lis_dep IN VARCHAR2 ) IS
...
-- PGV moved types end
	...
	END IF;
	END LOOP;
	FOR ws_des_tip IN ( SELECT cam_keycam, cam_descam FROM LABPROD.glcocamp
	WHERE cam_keytab = 'nmlonomi' ) LOOP
