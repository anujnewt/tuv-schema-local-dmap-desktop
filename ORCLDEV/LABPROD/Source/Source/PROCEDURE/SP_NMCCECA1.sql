CREATE OR REPLACE NONEDITIONABLE PROCEDURE "LABPROD"."SP_NMCCECA1" (ws_nom_rep IN VARCHAR2,ws_ide_pcc IN VARCHAR2,wn_key_usu IN  NUMBER,ws_key_men IN VARCHAR2,ws_hor_reg  IN VARCHAR2,ws_lis_per  IN VARCHAR2,ws_lis_pro IN VARCHAR2,ws_lis_dep IN VARCHAR2 ) IS
...
EXCEPTION
 WHEN NO_DATA_FOUND THEN
  ...
  FROM LABPROD.nmwkmovt, LABPROD.glwkrang
	...
	INTO wn_mov_emp FROM LABPROD.nmwkmovt
	WHERE mov_keyper IN (
	...
	AND ran_keyusu = wn_key_usu
	AND ran_keypro IS NOT NULL )
	AND mov_ca1aux IS NULL;
EXCEPTION
 WHEN NO_DATA_FOUND THEN
  NULL;
  END;
  BEGIN SELECT  COUNT( DISTINCT mov_keyemp ) alias1
	INTO wn_sum_pro FROM LABPROD.nmwkmovt
	WHERE mov_keyper IN (
	SELECT ran_keyper FROM LABPROD.glwkrang
	WHERE ran_nomrep = ws_nom_rep
	AND ran_idepcc = ws_ide_pcc
...
EXCEPTION
 WHEN NO_DATA_FOUND THEN
  ...
  END;
  BEGIN SELECT  COUNT( DISTINCT mov_keyemp ) alias1
	INTO wn_sum_per FROM LABPROD.nmwkmovt
...
EXCEPTION
 WHEN NO_DATA_FOUND THEN
  ...
  END IF;
	...
	LABPROD.sp_nomdes(ws_mov_dep, 1,ws_des_dep);
...
EXCEPTION
 WHEN NO_DATA_FOUND THEN
  ...
  ws_ant_per:=ws_mov_per;
	...
	INTO ws_des_not FROM LABPROD.nmlonomi
