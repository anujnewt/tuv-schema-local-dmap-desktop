CREATE OR REPLACE NONEDITIONABLE PROCEDURE "LABPROD"."SP_NMLSTHIS" (ws_nom_rep IN VARCHAR2,ws_ide_pcc IN VARCHAR2,wn_key_usu IN  NUMBER,ws_key_men IN VARCHAR2,ws_hor_reg IN VARCHAR2,ws_lis_per IN VARCHAR2,ws_lis_pro IN VARCHAR2 ) IS
...
-- PGV moved types end
	...
	INTO wn_tot_reg FROM LABPROD.nmloperi
	WHERE per_keypro IN (
...
EXCEPTION
 WHEN NO_DATA_FOUND THEN
  ...
  ws_etq_001:='........';
	...
	wn_dsp_005:=0;
	wn_dsp_006:=0;
	LABPROD.sp_glGetDsp('nmwkmovt', 'mov_keypro', ws_key_men,wn_dsp_001);
	LABPROD.sp_glGetDsp('nmwkmovt', 'mov_keyper', ws_key_men,wn_dsp_002);
	LABPROD.sp_glGetDsp('nmwkmovt', 'mov_keycon', ws_key_men,wn_dsp_003);
	LABPROD.sp_glGetDsp('nmwkmovt', 'mov_cantid', ws_key_men,wn_dsp_004);
	LABPROD.sp_glGetDsp('nmwkmovt', 'mov_import', ws_key_men,wn_dsp_005);
	LABPROD.sp_glGetDsp('nmwkmovt', 'mov_keynom', ws_key_men,wn_dsp_006);
	ws_ant_con:='@@@';
	wn_ant_nom:=-9999;
	ws_ant_per:='@@@';
	wn_ant_pro:=-9999;
...
EXCEPTION
 WHEN NO_DATA_FOUND THEN
  ...
  wn_pri_mer:=(wn_pri_mer+1);
	...
	INTO ws_des_pro, ws_key_cia FROM LABPROD.nmloproc
	WHERE pro_keypro = wn_his_pro;
EXCEPTION
 WHEN NO_DATA_FOUND THEN
  NULL;
...
EXCEPTION
 WHEN NO_DATA_FOUND THEN
  ...
  ws_des_cia:=ws_des_cor;
	BEGIN SELECT  COUNT( DISTINCT his_keyemp ) alias1
	INTO wn_tot_emp FROM LABPROD.nmlohism
...
EXCEPTION
 WHEN NO_DATA_FOUND THEN
  ...
  ws_ant_con:=ws_his_con;
	...
	LABPROD.sp_glfechor(wd_fec_ini, ws_hor_act);
