create or replace  function  labprod."fn_basedatos_emp"  ( keyemp numeric ) returns varchar as $body$
declare
...
base := 'TELECOM';
for q_empleado in (
select case when nullif(pam_cvesec::text, '') is null then 'TVNOMINA' else 'TELECOM' end base, emp_keypro,emp_status
