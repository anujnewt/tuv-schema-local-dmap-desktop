create or replace procedure labprod."sp_nmlstnm2a"  (ws_nom_rep varchar,ws_des_rep varchar,ws_ide_pcc varchar,wn_key_usu numeric,ws_key_men varchar,ws_hor_reg varchar,wn_key_pro numeric,wn_tip_nom numeric,ws_key_per varchar ) as $body$
declare
...
c_nmlstnom record;
begin
call labprod.sp_glfechor (ws_fec_act, ws_hor_act);
--ws_fec_act := '04-SEP-2002';
--ws_hor_act := '11:17:38';
wn_tot_reg:=-1;
...
into strict ws_key_cia from labprod.nmloproc
...
into strict ws_des_cor, ws_rfc_cia from labprod.nmlocias
...
into strict ws_key_tab from labprod.glcopams
where pam_keypar = (
select distinct pam_folini from labprod.glcopams
where pam_keypar='00'
...
ws_con_his:= null;
begin select pam_folini
into strict ws_con_his from labprod.glcopams
where pam_keypar = (
select distinct pam_folini from labprod.glcopams
where pam_keypar='00'
and pam_cvesec='repnom')
...
ws_opc_003:= null;
begin select pam_folini
into strict ws_opc_003 from labprod.glcopams
where pam_keypar = (
select distinct pam_folini from labprod.glcopams
where pam_keypar='00'
and pam_cvesec='repnom')
...
ws_opc_004:= null;
begin select pam_folini
into strict ws_opc_004 from labprod.glcopams
where pam_keypar = (
select distinct pam_folini from labprod.glcopams
where pam_keypar='00'
and pam_cvesec='repnom')
...
ws_opc_005:= null;
begin select pam_folini
into strict ws_opc_005 from labprod.glcopams
where pam_keypar = (
select distinct pam_folini from labprod.glcopams
where pam_keypar='00'
and pam_cvesec='repnom')
...
ws_opc_006:= null;
begin select pam_folini
into strict ws_opc_006 from labprod.glcopams
where pam_keypar = (
select distinct pam_folini from labprod.glcopams
where pam_keypar='00'
and pam_cvesec='repnom')
...
ws_opc_007:= null;
begin select pam_folini
into strict ws_opc_007 from labprod.glcopams
where pam_keypar = (
select distinct pam_folini from labprod.glcopams
where pam_keypar='00'
and pam_cvesec='repnom')
...
wn_dsp_009:=0;
wn_dsp_010:=0;
call labprod.sp_glnewdsp ('nmwkmovt', 'mov_keyemp', ws_key_men,wn_dsp_001);
call labprod.sp_glnewdsp ('nmwkmovt', 'mov_keydep', ws_key_men,wn_dsp_002);
call labprod.sp_glnewdsp ('nmwkmovt', 'mov_keypue', ws_key_men,wn_dsp_003);
call labprod.sp_glnewdsp ('nmwkmovt', 'mov_keycon', ws_key_men,wn_dsp_004);
call labprod.sp_glnewdsp ('nmwkmovt', 'mov_keyper', ws_key_men,wn_dsp_005);
call labprod.sp_glnewdsp ('nmwkmovt', 'mov_keynom', ws_key_men,wn_dsp_006);
call labprod.sp_glnewdsp ('nmwkmovt', 'mov_cantid', ws_key_men,wn_dsp_007);
call labprod.sp_glnewdsp ('nmwkmovt', 'mov_import', ws_key_men,wn_dsp_008);
call labprod.sp_glnewdsp ('nmwkmovt', 'emp_tipemp', ws_key_men,wn_dsp_009);
call labprod.sp_glnewdsp ('nmwkmovt', 'emp_keyloc', ws_key_men,wn_dsp_010);
wn_num_reg:=0;
wn_pct_act:=1;
...
ws_des_aux:= oracle.substr(ws_des_lis,41,9);
ws_key_dep:= 'HHHHHHHHH';
k:=1;
for c_nmlstnom in ( select mov_keyemp, mov_keycon, mov_cantid, mov_import, mov_codimp from labprod.nmwkmovt
...
into strict ws_nom_emp, ws_reg_rfc, wn_sal_dia, wn_sal_int, ws_key_dep, ws_key_pue, ws_tip_emp, ws_key_ims, ws_key_loc, ws_reg_ims from labprod.nmcoempl
...
begin select  distinct mov_keydep, mov_keypue, mov_ca2aux
into strict ws_dep_his, ws_pue_his, ws_loc_his from labprod.nmwkmovt
...
where dep_keydep=ws_key_dep  limit 1;
exception
when no_data_found then
null;
end;
ws_des_pue:='PUESTO NO EXISTE';
begin select oracle.substr(pue_despue,1,40)
into strict ws_des_pue from labprod.nmcopues
where pue_keypue=ws_key_pue  limit 1;
exception
when no_data_found then
null;
end;
...
into strict ws_des_loc from labprod.nmlolocp
where loc_keyloc=ws_key_loc  limit 1;
exception
when no_data_found then
null;
end;
...
begin select distinct pam_nompar
into strict ws_des_tip from labprod.glcopams
...
into strict ws_rfc_ims from labprod.nmloimss
...
if (k=3 ) then
update labprod.glwkcrys set cry_chr011=ws_key_con,cry_dec015=wn_imp_mov,cry_chr003=ws_des_con
where cry_nomrep=ws_nom_rep
and cry_idepcc=ws_ide_pcc
and cry_keyusu=wn_key_usu
and cry_numsec=-1
