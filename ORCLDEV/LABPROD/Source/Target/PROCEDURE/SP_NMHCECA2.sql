create or replace procedure labprod."sp_nmhceca2"  (ws_nom_rep varchar,ws_ide_pcc varchar,wn_key_usu numeric,ws_key_men varchar,ws_hor_reg varchar,ws_lis_per varchar,ws_lis_pro varchar,ws_lis_dep varchar ) as $body$
declare
...
and ran_keyusu = wn_key_usu
and nullif(ran_keypro::text, '') is not null )
and nullif(his_ca2aux::text, '') is null;
exception
when no_data_found then
null;
end;
...
call sp_nomdes (ws_his_dep, 2,ws_des_dep);
...
into strict ws_des_pro, ws_his_cia from labprod.nmloproc
