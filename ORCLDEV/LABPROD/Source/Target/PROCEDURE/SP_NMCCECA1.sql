create or replace procedure labprod."sp_nmcceca1"  (ws_nom_rep varchar,ws_ide_pcc varchar,wn_key_usu numeric,ws_key_men varchar,ws_hor_reg varchar,ws_lis_per varchar,ws_lis_pro varchar,ws_lis_dep varchar ) as $body$
declare
...
wn_pri_mer:=0;
wn_tot_tra:=0;
for c_nmlstcic in ( select mov_keypro, mov_keyper, mov_ca1aux, mov_codimp, mov_keycon,  sum(mov_cantid ) alias6,  sum(mov_import ) alias7,  count(* ) alias8
from labprod.nmwkmovt, labprod.glwkrang
where mov_keypro = ran_keypro
and mov_keyper = ran_keyper
and mov_ca1aux = ran_keydep
...
group by mov_ca1aux, mov_keypro, mov_keyper,  mov_codimp, mov_keycon
order by  mov_ca1aux,  mov_keypro, mov_keyper, mov_codimp, mov_keycon ) loop
wn_mov_pro :=c_nmlstcic.mov_keypro;
ws_mov_per :=c_nmlstcic.mov_keyper;
ws_mov_dep :=c_nmlstcic.mov_ca1aux;
...
into strict wn_mov_emp from labprod.nmwkmovt
where mov_keyper in (
...
and ran_keyusu = wn_key_usu
and nullif(ran_keypro::text, '') is not null )
and nullif(mov_ca1aux::text, '') is null;
exception
when no_data_found then
null;
end;
begin select  count( distinct mov_keyemp ) alias1
into strict wn_sum_pro from labprod.nmwkmovt
where mov_keyper in (
select ran_keyper from labprod.glwkrang
where ran_nomrep = ws_nom_rep
and ran_idepcc = ws_ide_pcc
...
begin select  count( distinct mov_keyemp ) alias1
into strict wn_sum_per from labprod.nmwkmovt
...
ws_des_dep:='No Existe Centro de costos';
else
if (ws_ant_dep != ws_mov_dep ) then
ws_des_dep:='No Existe';
call labprod.sp_nomdes (ws_mov_dep, 1,ws_des_dep);
...
into strict ws_des_not from labprod.nmlonomi
...
insert into labprod.glwkcrys( cry_nomrep,cry_idepcc,cry_keyusu,cry_chr001,cry_chr003,cry_chr004,cry_chr017,cry_chr018,cry_chr019,cry_chr010,  cry_chr011, cry_chr022,cry_chr024,cry_chr025,cry_chr005,cry_dec001,cry_dec002,cry_dec006,cry_chr026,cry_dec008,cry_dec009,cry_dat001,cry_dat002,cry_dat003,cry_chr027, cry_chr002, cry_chr028, cry_chr009,cry_dec010,cry_chr006,cry_chr007,cry_chr008,cry_dec007,cry_chr014, cry_chr015,cry_dec011,cry_dec012)
values (ws_nom_rep,ws_ide_pcc,wn_key_usu,ws_des_cia,ws_des_lis,ws_des_nom,ws_etq_001,ws_etq_002,ws_etq_003,ws_etq_004,ws_etq_005,ws_etq_006,ws_hor_act,ws_ant_con,ws_des_con,wn_tot_can,wn_tot_imp,wn_mov_pro,ws_mov_per,wn_mov_nom,wn_con_tar,wd_fec_ini,wd_fec_fin,ws_dia_act,ws_ant_cod,ws_lis_per,ws_mov_cia,ws_etq_007,wn_mov_emp,ws_lis_pro,ws_des_dep,ws_des_pro,wn_mov_emp,ws_ant_dep,ws_etq_008,wn_sum_pro,wn_sum_per); /* commit; */
end if;
end loop;
