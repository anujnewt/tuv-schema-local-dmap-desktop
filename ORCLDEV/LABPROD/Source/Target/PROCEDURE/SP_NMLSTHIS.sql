create or replace procedure labprod."sp_nmlsthis"  (ws_nom_rep varchar,ws_ide_pcc varchar,wn_key_usu numeric,ws_key_men varchar,ws_hor_reg varchar,ws_lis_per varchar,ws_lis_pro varchar ) as $body$
declare
...
into strict wn_tot_reg from labprod.nmloperi
where per_keypro in (
...
wn_dsp_005:=0;
wn_dsp_006:=0;
call labprod.sp_glgetdsp ('nmwkmovt', 'mov_keypro', ws_key_men,wn_dsp_001);
call labprod.sp_glgetdsp ('nmwkmovt', 'mov_keyper', ws_key_men,wn_dsp_002);
call labprod.sp_glgetdsp ('nmwkmovt', 'mov_keycon', ws_key_men,wn_dsp_003);
call labprod.sp_glgetdsp ('nmwkmovt', 'mov_cantid', ws_key_men,wn_dsp_004);
call labprod.sp_glgetdsp ('nmwkmovt', 'mov_import', ws_key_men,wn_dsp_005);
call labprod.sp_glgetdsp ('nmwkmovt', 'mov_keynom', ws_key_men,wn_dsp_006);
ws_ant_con:='______';
wn_ant_nom:=-9999;
ws_ant_per:='______';
wn_ant_pro:=-9999;
...
into strict ws_des_pro, ws_key_cia from labprod.nmloproc
where pro_keypro = wn_his_pro;
exception
when no_data_found then
null;
...
begin select  count( distinct his_keyemp ) alias1
into strict wn_tot_emp from labprod.nmlohism
...
call labprod.sp_glfechor (wd_fec_ini, ws_hor_act);
...
into strict wd_fec_ini, wd_fec_fin, wn_his_nom from labprod.nmloperi
...
values (ws_nom_rep,ws_ide_pcc,wn_key_usu,oracle.substr(ws_des_cor,1,60), ws_des_pro,ws_des_nom,ws_etq_001,ws_etq_002,ws_etq_003,ws_etq_004,ws_etq_005,ws_etq_006,ws_hor_act,ws_ant_con,ws_des_con,wn_tot_can,wn_tot_imp,wn_his_pro,ws_his_per,wn_his_nom,wn_con_tar,wd_fec_ini,wd_fec_fin,ws_dia_act,ws_ant_cod,oracle.substr(ws_des_cia,1,60),ws_key_cia,ws_etq_007,wn_tot_emp,ws_lis_pro,ws_lis_per,ws_des_lis,wn_his_emp); /* commit; */
end if;
end loop;end;
$body$
language plpgsql
