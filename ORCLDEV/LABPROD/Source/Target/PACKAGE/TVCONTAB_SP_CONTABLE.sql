create or replace procedure labprod.tvcontab_sp_contable (proceso labprod.nmlohism.his_keypro%type, periodo labprod.nmlohism.his_keyper%type) as $body$
declare
...
update labprod.nmloperi set per_persel = 'P' where per_keyper = dmap_extension.f_dmap_get_pkg_var('LABPROD' , 'TVCONTAB', 'PERIODO', 'varchar2', 'N')::varchar and per_keypro = dmap_extension.f_dmap_get_pkg_var('LABPROD' , 'TVCONTAB', 'PROCESO', 'number', 'N')::numeric;
/* commit; */
select per_keynom,per_keypol,pro_vercon
into strict nomina_temp, cvepoliza, cveversionproc from labprod.nmloperi
...
into strict dr_ciaori_temp from    labprod.glcopams
where   pam_keypar = (select  pam_folini
from    labprod.glcopams
where   pam_keypar = '00'
and     pam_cvesec = 'polcon')
and     pam_cvesec = 'OPCI11';
call dmap_extension.p_dmap_set_pkg_var('LABPROD' , 'TVCONTAB', 'DR_CIAORI', 'varchar2',(dr_ciaori_temp)::text, 'N');
-- dr_ciades tvwkpoli.pol_cia%type;  	-- cia destino para aplicar doble registro	522			opci12
select  coalesce(pam_folini, '522')
into strict dr_ciades_temp from    labprod.glcopams
where   pam_keypar = (select  pam_folini
from    labprod.glcopams
where   pam_keypar = '00'
and     pam_cvesec = 'polcon')
and     pam_cvesec = 'OPCI12';
call dmap_extension.p_dmap_set_pkg_var('LABPROD' , 'TVCONTAB', 'DR_CIADES', 'varchar2',(dr_ciades_temp)::text, 'N');
-- dr_concep nmlohism.his_keycon%type; -- cve concepto de comisiones					    	m39			opci13
select  coalesce(pam_folini, 'M39')
into strict dr_concep_temp from    labprod.glcopams
where   pam_keypar = (select  pam_folini
from    labprod.glcopams
where   pam_keypar = '00'
and     pam_cvesec = 'polcon')
and     pam_cvesec = 'OPCI13';
call dmap_extension.p_dmap_set_pkg_var('LABPROD' , 'TVCONTAB', 'DR_CONCEP', 'varchar2',(dr_concep_temp)::text, 'N');
-- dr_cta   	tvwkpoli.pol_cta%type;		-- cta para afectacion								      126			opci14
select  coalesce(pam_folini, '126')
into strict dr_cta_temp from    labprod.glcopams
where   pam_keypar = (select  pam_folini
from    labprod.glcopams
where   pam_keypar = '00'
and     pam_cvesec = 'polcon')
and     pam_cvesec = 'OPCI14';
call dmap_extension.p_dmap_set_pkg_var('LABPROD' , 'TVCONTAB', 'DR_CTA', 'varchar2',(dr_cta_temp)::text, 'N');
-- dr_scta   tvwkpoli.pol_scta%type;  	-- scta para afectacion											012002	opci15
select  coalesce(pam_folini, '012002')
into strict dr_scta_temp from    labprod.glcopams
where   pam_keypar = (select  pam_folini
from    labprod.glcopams
where   pam_keypar = '00'
and     pam_cvesec = 'polcon')
and     pam_cvesec = 'OPCI15';
call dmap_extension.p_dmap_set_pkg_var('LABPROD' , 'TVCONTAB', 'DR_SCTA', 'varchar2',(dr_scta_temp)::text, 'N');
-- -------------------------------------------------------------------------------------------------------------------
-- borra p??liza
delete from labprod.tvwkpoli where pol_keypol = cvepoliza;
/* commit; */
-- cuenta
select count(*) into strict total from labprod.nmlohism
...
where his_keyper = dmap_extension.f_dmap_get_pkg_var('LABPROD' , 'TVCONTAB', 'PERIODO', 'varchar2', 'N')::varchar
and his_keypro = dmap_extension.f_dmap_get_pkg_var('LABPROD' , 'TVCONTAB', 'PROCESO', 'number', 'N')::numeric
and (length(dep_refcon) = 52 or length(dep_refcon) = 26);
if total = 0 then
update labprod.nmloperi set per_persel = null where per_keyper = dmap_extension.f_dmap_get_pkg_var('LABPROD' , 'TVCONTAB', 'PERIODO', 'varchar2', 'N')::varchar and per_keypro = dmap_extension.f_dmap_get_pkg_var('LABPROD' , 'TVCONTAB', 'PROCESO', 'number', 'N')::numeric;
...
if (i mod dmap_extension.f_dmap_get_pkg_var('LABPROD' , 'TVCONTAB', 'REGS', 'INTEGER', 'N')::integer = 0) then
