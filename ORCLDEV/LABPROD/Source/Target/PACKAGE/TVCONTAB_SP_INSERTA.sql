create or replace procedure labprod.tvcontab_sp_inserta (datospf tvcontab_partefija, referencia labprod.nmcodeps.dep_refcon%type, ctaconcepto labprod.nmloconc.con_ctaref%type, seietu labprod.nmloconc.con_porcen%type, impcar labprod.nmlohism.his_import%type, impabo labprod.nmlohism.his_import%type, tipo labprod.tvwkpoli.pol_tipo%type) as $body$
declare
...
if tipo = dmap_extension.f_dmap_get_pkg_var('LABPROD' , 'TVCONTAB', 'MADRE', 'number', 'N')::numeric then
...
dr_scta_temp := dmap_extension.f_dmap_get_pkg_var('LABPROD' , 'TVCONTAB', 'DR_SCTA', 'varchar2', 'N')::varchar;
dr_cta_temp := dmap_extension.f_dmap_get_pkg_var('LABPROD' , 'TVCONTAB', 'DR_CTA', 'varchar2', 'N')::varchar;
-- ---------------------------------------------------------------------------------------------------------
...
if cia = dmap_extension.f_dmap_get_pkg_var('LABPROD' , 'TVCONTAB', 'DR_CIAORI', 'varchar2', 'N')::varchar then		-- opci compa??ia hija para aplicar doble registro
if datospf.keycon = dmap_extension.f_dmap_get_pkg_var('LABPROD' , 'TVCONTAB', 'DR_CONCEP', 'varchar2', 'N')::varchar then  -- opci clave concepto comisiones
insert into labprod.tvwkpoli values (datospf.keyemp, datospf.keypro, datospf.keycon, datospf.codimp, datospf.descon, -- cia,   neg,  cuenta, subcta,  ccostos,    icia,  top,
dmap_extension.f_dmap_get_pkg_var('LABPROD' , 'TVCONTAB', 'DR_CIADES', 'varchar2', 'N')::varchar, '01', dr_cta_temp, dr_scta_temp, '00000000', '000', '0', ietu, tot_car, tot_abo, datospf.keycia, datospf.keypol, datospf.cveban, datospf.forpag, datospf.keyben, datospf.comfam, tipo, datospf.fecmov);
