create or replace procedure labprod.dmap_tvautsaf_local_sp_datosgenerales_pa_aut (cliente varchar,keyemp varchar, recurp nmcoempl.emp_recurp%type, nombre varchar, paterno varchar, materno varchar, domicilio varchar, colonia varchar, ciudad varchar, municipio varchar, entidad varchar, codigopostal varchar, telefono varchar, clavebanco varchar, cuentabanco varchar, salariomensual varchar, fechaingreso timestamp(0), tipopago varchar, metodopago varchar, tipocontrato varchar, proceso varchar, localidad varchar, status varchar, fecha timestamp(0)) as $body$
declare
...
begin
select count(*) into strict existe_empleado
from saf_dgfa
...
begin
insert into saf_dgfa values (' ', cliente, recurp, keyemp, nombre, paterno, materno, domicilio, colonia,
