create or replace  function  usrsiho."sp_calimptiext"  (pi_entrada numeric, pi_salida numeric, pi_comida numeric, pi_capini numeric, pi_capfin numeric, ps_programa varchar, pl_keyfol numeric, pl_keypue varchar, pd_costopar numeric, pi_keytco numeric, pi_keyemp numeric, pd_fecgra timestamp(0), pi_detserial numeric, pi_hojatrab numeric) returns numeric as $body$
declare
...
ls_fecgra varchar(10);
li_diafes numeric(10);
li_mardiaf numeric(10);
...
ls_fecgra := pd_fecgra;/* dmap converted statement start */
--insert into borra(sec,campo1,campo2,campo3) values(18,'Pd_FecGra',0,pd_fecgra);
ls_fecgra := oracle. concat(substr(ls_fecgra,4,2), '/', oracle.substr(ls_fecgra,1,2), '/', oracle.substr(ls_fecgra,7,4)) ;/* dmap converted statement end */
--insert into borra(sec,campo1,campo2,campo3) values(18,'Ls_FecGra',0,ls_fecgra);
select count(*)
into strict li_diafes
from glcopams
...
-- ----------------------------------------
if li_diafes > 0 then
select case when coalesce((oracle.substr(det_auxca2,8,1))::numeric ,0)=1 then 1 else 0 end
into strict li_mardiaf
from holodettra
