create or replace procedure usrsiho."sp_hrpcanda1"  (pn_nomrep varchar,pn_idepcc varchar,pn_keyusu numeric,pn_keypro numeric) as $body$
declare
...
begin
---aedo 14/feb/05
select distinct ran_keycen into strict ws_feccob
...
select distinct ran_keynom into strict ws_keynom
from usrsiho.glwkrang
where ran_nomrep = pn_nomrep
and ran_idepcc = pn_idepcc
and ran_keyusu = pn_keyusu
and nullif(ran_keynom::text, '') is not null;
select distinct ran_keycat into strict ws_keycat
from usrsiho.glwkrang
where ran_nomrep = pn_nomrep
and ran_idepcc = pn_idepcc
and ran_keyusu = pn_keyusu
and nullif(ran_keycat::text, '') is not null;
select distinct ran_keypue into strict ws_keypue
from usrsiho.glwkrang
where ran_nomrep = pn_nomrep
and ran_idepcc = pn_idepcc
and ran_keyusu = pn_keyusu
...
-- emilio 23/02/06 agregue el sp_tointeger
...
and (ran_keycat)::numeric  = rec_ejerci
...
update usrsiho.holoreci_ter set rec_stsrec = 2
where rec_ejerci = ln_ejerci
and rec_keypro = pn_keypro
and rec_keyrec = ln_keyrec;/* dmap converted statement start */
-- nuevo 20/10/2003
-- his_ca1aux[4] = '2'
update usrsiho.nmlohism set his_ca1aux = oracle. concat(substr(his_ca1aux,1,3), '2', oracle.substr(his_ca1aux,5,16)
...
insert into usrsiho.holococa(coc_keyplz,coc_keycap,coc_numsec,coc_stspag,coc_keyrph,coc_keygdp,coc_hjatra,coc_reghja)
select coc_keyplz,coc_keycap,ln_num+1,'V',null,null,coc_hjatra,coc_reghja  from usrsiho.holococa
where coc_keyplz = ln_keyplz
and coc_keycap = ln_keycap
and coc_numsec = ln_num;
end loop;
...
--                 coc_keygdp = null,
--                 coc_stspag = 'V',
--                 coc_numsec = coc_numsec + 1;
--insert into holococa
--select * from holocanc;
--execute immediate 'TRUNCATE TABLE holocanc';
end if;
end loop;
-- eliminacion de ispt
...
and his_keycon in ('HPG','H53','H76')) loop
...
--               ;
insert into nmloamor(amo_keyemp, amo_keycon, amo_keypre, amo_refere, amo_keypro,
...
amo_keydep, amo_keypue, amo_keycat, amo_keyubi, amo_keyper,
amo_keynom, amo_numpag, 'E', 'CANCELACION',
