create or replace procedure usrsiho."sp_hpgimpre4"  (ws_nomrep varchar,ws_idepcc varchar, wn_keyusu numeric, ws_keyapr varchar, wn_keypro numeric, wn_tiprec numeric, wn_lstemp numeric, wd_fec_pag timestamp(0)) as $body$
declare
...
--wd_fec_pag := to_timestamp(ws_fec_pag,'MM/DD/yyyy');
...
join usrsiho.glcopams on pam_cvesec=rec_keyapr and pam_keypar='H2'
...
ws_sitfis := rtrim(oracle.substr(rec.emp_ca2aux::text,1,8));/* dmap converted statement end */
