create or replace procedure usrsiho."sp_nmreppre"  (ws_nom_rep varchar, ws_ide_pcc varchar, wn_key_usu numeric, ws_key_men varchar, ws_hor_reg varchar, ws_sta_tu1 varchar , ws_sta_tu2 varchar,ws_sta_tu3 varchar, ws_sta_tu4 varchar, wn_key_pro numeric, ws_des_pro varchar ) as $body$
declare
...
where pre_keypre in ( select ran_keydep from usrsiho.glwkrang
...
perform dbms_output.put_line('dentro de ciclo..');
...
if wn_dsp_013 = 1 then ws_per_ini := null; end if;
...
perform dbms_output.put_line('insert..');
