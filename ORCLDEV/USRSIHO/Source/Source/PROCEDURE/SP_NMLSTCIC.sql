CREATE OR REPLACE EDITIONABLE PROCEDURE "USRSIHO"."SP_NMLSTCIC" (ws_nom_rep IN VARCHAR2, ws_ide_pcc IN VARCHAR2,
...
-- PGV moved types end
  ...
  -- Orden en reporte
  --variables
  intOrdConcep NUMBER(10);
  intMaxOrdConcep NUMBER(10);
  ...
  BEGIN
	...
	BEGIN
		SELECT  NVL(MAX(pam_folfin), 0) + 1
		INTO    intMaxOrdConcep
	...
	BEGIN
        ...
        FROM usrsiho.nmwkmovt
        WHERE mov_keypro IN
        ( SELECT ran_keypro FROM glwkrang
            WHERE ran_nomrep = ws_nom_rep
            AND ran_idepcc = ws_ide_pcc
            AND ran_keyusu = wn_key_usu
            AND ran_keypro IS NOT NULL )
    ...
    ws_hor_act := TO_CHAR (SYSDATE, 'HH24:MI:SS');
    UPDATE glcoresu SET res_numreg = wn_num_reg, res_fecfin = TRUNC(SYSDATE),
