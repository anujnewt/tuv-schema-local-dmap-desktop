CREATE OR REPLACE EDITIONABLE PROCEDURE "USRSIHO"."SP_HRPCANDA1" (pn_nomrep VARCHAR2,pn_idepcc VARCHAR2,pn_keyusu NUMBER,pn_keypro NUMBER) IS
...
BEGIN
---aedo 14/feb/05
              SELECT distinct ran_keycen INTO ws_feccob
              ...
              SELECT distinct ran_keynom INTO ws_keynom
                FROM USRSIHO.glwkrang
               WHERE ran_nomrep = pn_nomrep
                 AND ran_idepcc = pn_idepcc
                 AND ran_keyusu = pn_keyusu
                 AND ran_keynom IS NOT NULL;
              SELECT distinct ran_keycat INTO ws_keycat
                FROM USRSIHO.glwkrang
               WHERE ran_nomrep = pn_nomrep
                 AND ran_idepcc = pn_idepcc
                 AND ran_keyusu = pn_keyusu
                 AND ran_keycat IS NOT NULL;
              SELECT distinct ran_keypue INTO ws_keypue
                FROM USRSIHO.glwkrang
               WHERE ran_nomrep = pn_nomrep
                 AND ran_idepcc = pn_idepcc
                 AND ran_keyusu = pn_keyusu
...
-- Emilio 23/02/06 agregue el sp_tointeger
  FOR rec3 IN (SELECT distinct rec_keyrec, rec_keyemp, per_keyper, rec_numemi, per_nummes, rec_ejerci
           ...
           WHERE ran_nomrep = pn_nomrep
             ...
             AND to_number(ran_keycat) = rec_ejerci
...
-- Nuevo 20/10/2003
-- HIS_CA1AUX[4] = '2'
      UPDATE USRSIHO.nmlohism SET HIS_CA1AUX = SUBSTR(HIS_CA1AUX,1,3) ||'2'|| SUBSTR(HIS_CA1AUX,5,16)
