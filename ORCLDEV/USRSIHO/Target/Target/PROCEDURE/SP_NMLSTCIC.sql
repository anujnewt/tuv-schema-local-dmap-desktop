create or replace procedure usrsiho."sp_nmlstcic"  (ws_nom_rep varchar, ws_ide_pcc varchar, wn_key_usu numeric, ws_key_men varchar, ws_hor_reg varchar,ws_lis_per varchar) as $body$
declare
...
-- ig- andi, musicos y eje
-- orden en reporte
--variables
intordconcep numeric(10);
intmaxordconcep numeric(10);
c_etiqueta cursor for
select cam_keycam, cam_descor
from usrsiho.glcocamp
where cam_keytab = 'nmwkmovt';
...
c_tot_emp cursor for
select count(unique mov_keyemp) totemp
...
select per_fecini, per_fecfin, per_keynom, per_nu4aux, per_fecpag
...
and ran_idepcc = ws_ide_pcc
and ran_keyusu = wn_key_usu;
c_nmlstcic cursor for
select mov_keycon, sum(mov_cantid) suma_can ,sum(mov_import) suma_imp, mov_keypro, mov_keyper,
mov_codimp, count(*) total
, coalesce(pam_folfin, intmaxordconcep) pam_folfin  --ig- andi, musicos y eje
--ig- andi, musicos y eje
from usrsiho.nmwkmovt
left join glcopams on  pam_keypar = 'RPTS' and pam_cvesec = 'ORDN'
and pam_nompar = ws_nom_rep and pam_folini = mov_keycon
where mov_keypro in ( select ran_keypro from usrsiho.glwkrang
where ran_nomrep = ws_nom_rep
and ran_idepcc = ws_ide_pcc
and ran_keyusu = wn_key_usu
and nullif(ran_keypro::text, '') is not null )
and mov_keyper in ( select ran_keyper from usrsiho.glwkrang
where ran_nomrep = ws_nom_rep
and ran_idepcc = ws_ide_pcc
and ran_keyusu = wn_key_usu
and nullif(ran_keyper::text, '') is not null )
group by mov_keypro, mov_keyper, mov_codimp, mov_keycon, pam_folfin
order by mov_keypro, mov_keyper, mov_codimp, mov_keycon, pam_folfin;
cur refcursor;
begin
-- numero maximo para mantener el orden actual
begin
select  coalesce(max(pam_folfin::numeric), 0) + 1
into strict    intmaxordconcep
from    usrsiho.glcopams
where   pam_keypar = 'RPTS'
and pam_cvesec = 'ORDN'
and pam_nompar = ws_nom_rep;
exception when no_data_found then intmaxordconcep := 1;
end;
-- termina orden en reporte
...
for rec5 in c_des_tip loop
ws_key_cam := rec5.cam_keycam;
ws_des_etq1 := rec5.cam_descam;
...
for rec6 in c_desplieg loop
ws_key_cam := rec6.rec_keycam;
ws_dsp_cam := rec6.rec_despli;
...
wn_pri_mer := 0;
wn_tot_tra := 0;
-- para obtener el total de empleados a procesar
--open cur for
begin
select count(unique mov_keyemp )
into strict wn_mov_emp
from usrsiho.nmwkmovt
where mov_keypro in ( select ran_keypro from glwkrang
where ran_nomrep = ws_nom_rep
and ran_idepcc = ws_ide_pcc
and ran_keyusu = wn_key_usu
and nullif(ran_keypro::text, '') is not null )
and mov_keyper in ( select ran_keyper from glwkrang
where ran_nomrep = ws_nom_rep
and ran_idepcc = ws_ide_pcc
and ran_keyusu = wn_key_usu
and nullif(ran_keyper::text, '') is not null );
exception when no_data_found then wn_mov_emp := 0;
end;
-- define cursor principal -------------------------------------------------
for rec7 in c_nmlstcic loop
ws_mov_con := rec7.mov_keycon;
wn_tot_can := rec7.suma_can;
wn_tot_imp := rec7.suma_imp;
wn_mov_pro := rec7.mov_keypro;
ws_mov_per := rec7.mov_keyper;
ws_cod_imp := rec7.mov_codimp;
wn_con_tar := rec7.total;
intordconcep := rec7.pam_folfin;
if wn_pri_mer = 0 then
-- extrae el nombre de la compania corporativa
ws_des_cor := 'CORPORATIVO NO REGISTRADO ...';
for rec8 in c_descor loop
ws_key_cia := rec8.pro_keycia;
ws_des_cor := 'CORPORATIVO NO REGISTRADO ...';
...
ws_des_pro := 'No Existe Proceso...';
ws_mov_cia := '..';
for rec9 in c_for_pro loop
ws_des_pro := rec9.pro_despro;
ws_mov_cia := rec9.pro_keycia;
end loop;
ws_des_cia := 'Compania No existe...';
for rec10 in c_des_cia loop
ws_des_cia := rec10.cia_descia;
end loop;
for rec11 in c_lis_pro loop
ws_lis_pro := rec11.ran_keycen;
end loop;
for rec12 in c_tot_emp loop
wn_tot_emp := rec12.totemp;
end loop;
...
ws_des_con := 'No Existe Concepto...';
for rec13 in c_for_con loop
ws_des_con := rec13.con_descon;
end loop;
...
wd_fec_ini:= trunc(clock_timestamp());
wd_fec_fin:= trunc(clock_timestamp());
for rec14 in c_for_per loop
wd_fec_ini := rec14.per_fecini;
wd_fec_fin := rec14.per_fecfin;
wn_mov_nom := rec14.per_keynom;
wn_numemi := rec14.per_nu4aux;
wd_fecpag := rec14.per_fecpag;
end loop;
for rec15 in c_per_tot loop
wn_tot_emp := rec15.totemp;
end loop;
...
ws_des_nom := 'No Existe Nomina...';
for rec16 in c_for_nom loop
ws_des_nom := rec16.nom_destip;
end loop;
for rec17 in c_lis_nom loop
ws_lis_nom := rec17.ran_keycat;
end loop;
...
cry_chr004, cry_chr017, cry_chr018, cry_chr019, cry_chr010,
cry_chr011, cry_chr022, cry_chr024, cry_chr025, cry_chr005,
cry_dec001, cry_dec002, cry_dec006, cry_chr026, cry_dec008,
cry_dec009, cry_dat001, cry_dat002, cry_dat003, cry_chr027,
cry_chr002, cry_chr028, cry_chr009, cry_dec010, cry_chr006,
cry_chr007, cry_chr008, cry_chr012, cry_dec007, cry_dat004,
cry_chr044
, cry_dec003 --ig- andi, musicos y eje
)
values (
ws_nom_rep, ws_ide_pcc, wn_key_usu, ws_des_cor, ws_des_lis,
ws_des_nom, ws_etq_001, ws_etq_002, ws_etq_003, ws_etq_004,
ws_etq_005, ws_etq_006, ws_hor_act, ws_ant_con, ws_des_con,
wn_tot_can, wn_tot_imp, wn_mov_pro, ws_mov_per, wn_mov_nom,
wn_con_tar, wd_fec_ini, wd_fec_fin, ws_dia_act, ws_ant_cod,
ws_des_cia, ws_mov_cia, ws_etq_007, wn_tot_emp, ws_lis_pro,
ws_lis_per, ws_des_pro, ws_lis_nom, wn_mov_emp, wd_fecpag,
wn_numemi
, intordconcep --ig- andi, musicos y eje
);
-- fin del foreach principal
end loop;
-- actualiza la tabla de monitoreo la finalizacion del proceso
ws_hor_act := to_char(clock_timestamp(), 'HH24:MI:SS');
update glcoresu set res_numreg = wn_num_reg, res_fecfin = trunc(clock_timestamp()),
res_horfin = ws_hor_act, res_status = 'T'
where res_idepro = ws_nom_rep
and res_idepcc = ws_ide_pcc
and res_keyusu = wn_key_usu
and res_fecini = trunc(clock_timestamp())
