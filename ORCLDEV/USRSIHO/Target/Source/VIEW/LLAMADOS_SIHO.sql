CREATE OR REPLACE FORCE EDITIONABLE VIEW "USRSIHO"."LLAMADOS_SIHO" ("CENTRO_COSTOS", "DES_CENCOS", "TIPO_PROGRAMA", "DES_PROGRAMA", "CVE_ACTIVIDAD", "DES_ACTIVIDAD", "CODIGO_HON", "NOMBRE_ARTISTICO", "PERSONAJE", "NOMBRE_PRODUCTOR", "CVE_PROCESO", "DES_PROCESO", "FOLIO_CONTRATO", "CVE_TIPOCONTRATO", "DES_TIPOCONTRATO", "LLAMADO", "FECHA_LLAMADO", "HRA_LLAMADO", "FORO_LOCACION", "FECHA_CAPLLAM", "HOJA_TRABAJO", "STATUS_HOJATRAB", "SINDICATO", "FECHA_GRABACION", "FECHA_CAPHOJA", "HORAENT", "HORASAL", "CAPITULOS", "TABULADOR", "HRS_TRABAJADAS", "HRS_EXTRAS", "TPO_INCIDENCIA", "DES_TPOINCIDENCIA", "COSTO_UNITARIO", "DES_SINCENCOS") AS 
  SELECT x1.enc_keydep ,
         ...
         fn_diftiempo(((NVL(substr(x0.det_hraent,1,2),0)* 60) + NVL (substr(x0.det_hraent,4,2),0)) ,((NVL (substr(x0.det_hrasal,1,2),0)* 60) + NVL (substr(x0.det_hrasal,4,2),0))),
         CASE
            WHEN ((x0.det_keyemp IS NULL ) OR (x0.det_keyfol IS NULL ) )  THEN 0
            WHEN (x0.det_keyemp IS NOT NULL )  THEN fn_hocalctiextsias(((NVL (substr(x0.det_hraent,1,2) ,0 )* 60) + NVL (substr(x0.det_hraent,4,2) ,0 )) ,((NVL (substr(x0.det_hrasal,1,2),0 )* 60) + NVL (substr(x0.det_hrasal,4,2) ,0 )) ,60 ,NVL(x0.det_capini ,0 ),NVL (x0.det_capfin ,0 ),x0.det_keydep,x0.det_keyfol ,NVL (x3.con_keypue ,'X' ),NVL (x3.con_cosuni,0 ),x0.det_keytco ,x0.det_keyemp )
         END ,
         x0.det_tipinc ,
         CASE
         ...
         CASE
          ...
          END ,
         x1.enc_desscc
    FROM usrsiho.holodettra x0 ,
         usrsiho.holoenctra x1 ,
	     usrsiho.nmcoempl x2 ,
	     usrsiho.holocont x3 ,
         usrsiho.glcopams x4 ,
         usrsiho.glcopams x5 ,
