CREATE OR REPLACE EDITIONABLE PROCEDURE "USRSIHO"."SP_HPGIMPRE3" (ws_nomrep IN VARCHAR2,ws_idepcc IN VARCHAR2,wn_keyusu IN NUMBER,
                              ws_keyapr IN VARCHAR2,wn_keypro IN NUMBER,wn_keynom IN NUMBER,
                              wn_numemi IN NUMBER,wn_tiprec IN NUMBER,wn_lstemp IN NUMBER) IS
...
--- DEFINE VAL VARVARCHAR2(99);
   ...
   wn_keysec  NUMBER(10);  -- cry_dec025    Variable nueva por JDCM
...
BEGIN
  ...
  DBMS_OUTPUT.put_line('inicio..');
   ...
   FOR rec IN (SELECT rec_keyrec,emp_keyemp,emp_nomemp,emp_regrfc,pam_nompar,nom_destip,
           ...
           join usrsiho.nmloperi on rec_keypro=per_keypro AND rec_keynom=per_keynom AND rtrim(rec_keyapr)=rtrim(per_nu3aux)
              AND rec_numemi=rtrim(per_nu4aux)
           join usrsiho.glcopams on rtrim(pam_cvesec)=rtrim(rec_keyapr) and pam_keypar='H2' -- areas de produccion
           ...
           WHERE rtrim(rec_keyapr)=rtrim(ws_keyapr)
           ...
           join usrsiho.nmcoempl on rec_keyemp=emp_keyemp
           join usrsiho.nmlonomi on rec_keynom=nom_keynom
           join usrsiho.nmloperi on rec_keypro=per_keypro and rec_keynom=per_keynom AND rtrim(rec_keyapr)=per_nu3aux
 ...
 DBMS_OUTPUT.put_line('consulta principal 1..');
--- LET VAL = wn_numrec;
...
--Verifico que no exista descripcion opcional por opcis
            ...
            ws_sitfis := rtrim(rec.emp_ca2aux);
...
-----------------------------------------------------------------------------
    DBMS_OUTPUT.put_line('consulta principal..');
...
--- RETURN VAL WITH RESUME;
  DBMS_OUTPUT.put_line('inserta else..');
