create or replace procedure feci."feci_obtener_tipo_cambio_rcb_man_pr"  ( p_fecha_tc varchar, p_codigo_mon varchar ) as $body$
declare
...
from feci_tipo_cambio_cat
where fec_fecha_tc = (select max(fec_fecha_tc) from feci_tipo_cambio_cat)
and cod_moneda = p_codigo_mon
fetch next 1 rows only;
...
from feci_tipo_cambio_cat
where fec_fecha_tc = (select max(fec_fecha_tc) from feci_tipo_cambio_cat where cod_moneda = 'USD')
and cod_moneda = 'USD'
fetch next 1 rows only;
