create or replace procedure fecxc."feci_obtener_recibos_pr"  ( p_ver varchar, p_fecha_inicio timestamp(0), p_fecha_fin timestamp(0), p_tipo_recibo varchar default null, p_empresa varchar default null, p_folio_inicial numeric default null, p_folio_final numeric default null, p_clase_cliente varchar default null) as $body$
declare
...
v_query :=  concat('SELECT * FROM FECXC.FECI_RECIBOS_VW WHERE COD_ESTADO_RECIBO IN ( ', p_ver , ' )') ; /* dmap converted statement end *//* dmap converted statement start *//* dmap converted statement */
