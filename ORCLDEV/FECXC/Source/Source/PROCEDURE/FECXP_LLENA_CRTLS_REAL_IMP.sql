CREATE OR REPLACE EDITIONABLE PROCEDURE "FECXC"."FECXP_LLENA_CRTLS_REAL_IMP" 
AS
...
BEGIN
  v_contador_meses :=1;
  select NVL(to_number(max (mes)),0) into v_meses_a_actualizar from  FECXC.FECXP_IMP_DAT_HIST where procesado=0 AND TIPO_IMPORTACION='R';
...
INSERT INTO FECXC.FECXP_REAL_CARATULA_IMPNS (
    ...
    SELECT    E.ID_EMP , E.DESC_EMP , V_ID_SESION , TO_NUMBER (TO_CHAR (D.FECHA, 'YYYY')), D.MES, D.MONEDA_IMP, M.TIPO_CAMBIO,
    ...
    --AND     E.E_CODIGO = E.E_CODIGO--NVL (V_E_CODIGO, E.E_CODIGO)
   ...
   FOR c_meses IN 1..v_meses_a_actualizar LOOP
               DBMS_OUTPUT.PUT_LINE('INSERTANDO EL SALDO FINAL DEL MES ANTERIOR    '||to_char((v_contador_meses-1))||'COMO SALDO INICIAL DEL MES ACTUAL' ||to_char(v_contador_meses));
                 IF v_contador_meses > 1 THEN  ---NO BORRA EL SALDO INICIAL DE ENERO
                     ...
                     DELETE FECXC.FECXP_REAL_CARATULA_IMPNS
