CREATE OR REPLACE EDITIONABLE PROCEDURE "FECXC"."FECXP_LLENA_CARATULA_FORECAST" (
	V_MES_DESDE IN INT,
	V_PERIODO IN INT,
...
--== FORECAST ==--
	...
	SELECT	A.E_CODIGO, A.DES_EMPRESA,  V_ID_SESION, V_PERIODO, A.MES,
			A.MONEDA, A.TIPO_CAMBIO, B.CLA_FE_ID, SUBSTR (B.CLA_FE_DES, 1, 25) CLA_FE_DES, SUM (A.IMPORTE_LINEA * TO_NUMBER (B.CLA_ATRIBUTO5)), 'CH'
	...
	INSERT	INTO FECXP_REAL_CARATULA (E_CODIGO, DES_EMPRESA, ID_SESION_RC, PERIODO, MES, MONEDA, TIPO_CAMBIO, CLA_FE_ID, CLA_FE_DES, IMPORTE_LINEA,TIPO_CARATULA)
    SELECT	PS.E_CODIGO, E.DES_EMPRESA, V_ID_SESION, V_PERIODO, PS.MESCOD, M.MON_ORACLE, M.TIPO_CAMBIO, C.CLA_FE_ID, SUBSTR (CF.CLA_FE_DES, 1, 25), SUM (PS.IMPORTE_LINEA * TO_NUMBER (CF.CLA_ATRIBUTO5)), 'CH'
	...
	AND		D.E_EMPRESA_IMP = NVL (V_E_CODIGO, D.E_EMPRESA_IMP);
	...
	*/
	LOOP
        ...
        INSERT	INTO FECXP_REAL_CARATULA (
				E_CODIGO, DES_EMPRESA, ID_SESION_RC, PERIODO, MES, MONEDA, TIPO_CAMBIO, CLA_FE_ID, CLA_FE_DES, IMPORTE_LINEA, TIPO_CARATULA)
		SELECT	E_CODIGO, DES_EMPRESA, ID_SESION_RC, PERIODO, MES, MONEDA, TIPO_CAMBIO, 'SF COIN', 'SDO FINAL COINVERSION', SUM (NVL(IMPORTE_LINEA,0)), 'CO'
		FROM	FECXP_CLASIFICACION_FE CF,
				FECXP_REAL_CARATULA C
		WHERE	C.PERIODO = V_PERIODO
		AND		C.MES = V_MES_ACT
        ...
        INSERT	INTO FECXP_REAL_CARATULA (
				E_CODIGO, DES_EMPRESA, ID_SESION_RC, PERIODO, MES, MONEDA, TIPO_CAMBIO, CLA_FE_ID, CLA_FE_DES, IMPORTE_LINEA, TIPO_CARATULA)
		SELECT	E_CODIGO, DES_EMPRESA, ID_SESION_RC, PERIODO, MES, MONEDA, TIPO_CAMBIO, 'SF INV', 'SDO FINAL INVERSION', SUM (NVL(IMPORTE_LINEA,0)), 'IN'
		FROM	FECXP_CLASIFICACION_FE CF,
				FECXP_REAL_CARATULA C
		WHERE	C.PERIODO = V_PERIODO
		AND		C.MES = V_MES_ACT
		...
		SELECT	E_CODIGO, DES_EMPRESA, ID_SESION_RC,
				CASE WHEN MES = 12 THEN PERIODO + 1 ELSE PERIODO END,
				CASE WHEN MES = 12 THEN 1 ELSE MES + 1 END, MONEDA, TIPO_CAMBIO,
				'SI COIN', 'SDO INICIAL COINVERSION', NVL(IMPORTE_LINEA,0) IMPORTE_LINEA, 'CO'
		FROM	FECXP_REAL_CARATULA
		WHERE	PERIODO = V_PERIODO
		AND		MES = V_MES_ACT
		...
		SELECT	E_CODIGO, DES_EMPRESA, ID_SESION_RC,
				CASE WHEN MES = 12 THEN PERIODO + 1 ELSE PERIODO END,
				CASE WHEN MES = 12 THEN 1 ELSE MES + 1 END, MONEDA, TIPO_CAMBIO,
				'SI INV', 'SDO INICIAL INVERSION', NVL(IMPORTE_LINEA,0) IMPORTE_LINEA, 'IN'
		FROM	FECXP_REAL_CARATULA
		WHERE	PERIODO = V_PERIODO
		AND		MES = V_MES_ACT
...
END FECXP_LLENA_CARATULA_FORECAST;
/
