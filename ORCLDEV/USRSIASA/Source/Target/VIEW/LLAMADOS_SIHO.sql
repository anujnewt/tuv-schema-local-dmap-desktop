...
usrsiho.fn_diftiempo(((coalesce(oracle.substr(x0.det_hraent,1,2),0)* 60) + coalesce(oracle.substr(x0.det_hraent,4,2),0)) ,((coalesce(oracle.substr(x0.det_hrasal,1,2),0)* 60) + coalesce(oracle.substr(x0.det_hrasal,4,2),0))),
case
when ((nullif(x0.det_keyemp::text, '') is null ) or (nullif(x0.det_keyfol::text, '') is null ) )  then 0
when(nullif(x0.det_keyemp::text, '') is not null )  then usrsiho.fn_hocalctiextsias(((coalesce(oracle.substr(x0.det_hraent,1,2) ,0 )* 60) + coalesce(oracle.substr(x0.det_hraent,4,2) ,0 )) ,((coalesce(oracle.substr(x0.det_hrasal,1,2),0 )* 60) + coalesce(oracle.substr(x0.det_hrasal,4,2) ,0 )) ,60 ,coalesce(x0.det_capini ,0 ),coalesce(x0.det_capfin ,0 ),x0.det_keydep,x0.det_keyfol ,coalesce(x3.con_keypue ,'X' ),coalesce(x3.con_cosuni,0 ),x0.det_keytco ,x0.det_keyemp )
end ,
x0.det_tipinc ,
case
...
x1.enc_desscc
from nmloproc x9, nmloalde x8, nmcopues x7, nmcodeps x6, glcopams x5, glcopams x4, nmcoempl x2, holoenctra x1, holocont x3
left outer join holodettra x0 on (x3.con_keyemp = x0.det_keyemp and x3.con_keydep = x0.det_keydep and x3.con_keyfol = x0.det_keyfol)
where x0.det_num_id = x1.enc_num_id and x2.emp_keyemp = x0.det_keyemp    and x1.enc_keydep = x6.dep_keydep and x0.det_keypue = x7.pue_keypue and x1.enc_keydep = x8.ald_keydep and x1.enc_keypro = x9.pro_keypro and trim(both x4.pam_keypar) = 'H1' and x1.enc_keytpr = trim(both x4.pam_cvesec) and trim(both x5.pam_keypar) = 'H3' and x0.det_keytco = cast(trim(both x5.pam_cvesec) as integer) and x0.det_stsreg = 'V' and x0.det_inanda = 'N';/* dmap converted statement end */
