CREATE OR REPLACE FORCE NONEDITIONABLE VIEW "USRDRC"."PENDIUM_CONTROL_REPORTE_VW" ("ID_EMPRESA", "ID_META_ROW", "ID_FLEX_TBL", "NOM_FLEX", "SOLICITADO_POR", "VAL_C1", "FEC_PROG_ENTREGA", "FEC_ORDENAMIENTO", "FEC_EFECTIVA_ENTREGA", "VAL_C2", "VAL_C3", "VAL_C4", "VAL_C5", "VAL_C6", "VAL_C7", "VAL_C8", "VAL_C9", "VAL_C10", "NOM_RESPONSABLE", "RESPONSABLE") AS 
  SELECT
  META.ID_EMPRESA,
  META.ID_META_ROW,
  FLEX.ID_FLEX_TBL,
...
FROM
  ...
  LEFT JOIN USRDRC.DERCORP_ADD_CAMPO_CAT_VAL_TAB CAT ON CAT.ID_CATALOGO_VALOR = META.VAL_C2
  LEFT JOIN USRDRC.DERCORP_FLEX_TBLS_TAB FLEX ON FLEX.ID_FLEX_TBL = META.ID_FLEX_TBL
WHERE
  ...
  SELECT
    ...
    CASE ESC.IND_TIPO_ESCRITURA WHEN 'CP'
      THEN
      (CASE WHEN ESC.num_documentum_instr IS NULL OR ESC.fec_fecha IS NULL OR ESC.ind_delegado_por = -1 OR ESC.ind_delegado_por IS NULL THEN 'semaforo_red.png'
        ELSE 'semaforo_green.png' END)
    ELSE
      (CASE WHEN ESC.ind_requiere_proto = 0  THEN 'semaforo_gray.png'
        WHEN ESC.des_escritura IS null OR ESC.num_documentum_instr IS null
     OR ESC.fec_otorgamiento_instr IS null OR ESC.num_licenciado = -1 OR ESC.num_licenciado IS NULL THEN 'semaforo_red.png'
        ELSE 'semaforo_green.png' END)
    ...
    CASE ESC.IND_TIPO_ESCRITURA WHEN 'CP' THEN 'semaforo_gray.png'
      ELSE
        (CASE WHEN ESC.ind_requiere_inscr_rppc = 0 THEN 'semaforo_gray.png'
        WHEN fec_registro IS NULL THEN 'semaforo_red.png'
        ELSE 'semaforo_green.png' END)
   ...
   (SELECT (SELECT val_cat_val
        ...
        AND   id_catalogo_valor = (SELECT val_valor
                                   FROM USRDRC.dercorp_add_campo_valor_tab
                                   WHERE id_empresa = ESC.ID_EMPRESA
                                   AND id_add_campo = 500))AS NOM_RESPONSABLE,
    (SELECT (SELECT ID_CATALOGO_VALOR
                      FROM USRDRC.DERCORP_ADD_CAMPO_CAT_VAL_TAB
                      WHERE 1=1
...
FROM USRDRC.PENDIUM_ESCRITURA_PODER_TAB ESC
  LEFT JOIN USRDRC.DERCORP_ADD_CAMPO_CAT_VAL_TAB CAT ON ESC.IND_DELEGADO_POR = CAT.ID_CATALOGO_VALOR
  WHERE
  ESC.IND_STATUS=1;
