create or replace procedure usrdrc.dercorp_catalogs_pkg_get_catalog_elements_clasif_pr (catalogid integer, lstfilter varchar, lstcurrentids varchar,piinrolid numeric, resultset inout refcursor, lstclasif varchar, lstpais varchar) as $body$
declare
...
*/
--ecm 25 mayo 2016
lstquery :=  concat('select distinct(cat.id_catalogo_valor), cat.val_cat_val, vemp.id_clasificacion from dercorp_add_campo_cat_val_tab cat, dercorp_busqueda_view vemp where 1=1 and cat.val_cat_val = vemp.denom_actual and cat.id_catalogo = 1 and (cat.val_cat_val like ''% replace(', lstfilter , ', '''', ''%'') %'' or ' , lstcurrentids , ' like ''% cat.id_catalogo_valor %'') and vemp.id_empresa not in (' , lstin , ') ' , lstpais , ' ' , lstclasif , '  ORDER BY  VAL_CAT_VAL') ; /* dmap converted statement end *//* dmap converted statement start *//* dmap converted statement */
else
/*
