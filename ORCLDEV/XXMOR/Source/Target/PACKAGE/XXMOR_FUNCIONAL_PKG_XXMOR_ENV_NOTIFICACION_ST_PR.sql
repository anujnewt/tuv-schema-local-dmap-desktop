create or replace procedure xxmor.xxmor_funcional_pkg_xxmor_env_notificacion_st_pr ( p_id_solicitud integer, p_lineas varchar, p_id_notificacion integer ) as $body$
declare
c_mails record;
...
call xxmor_funcional_pkg_xxmor_html_email_pr(v_lista_mails_mkt, v_mail_subject, null, xxmor_funcional_pkg_xxmor_html_mail(p_id_solicitud, p_lineas, p_id_notificacion)  );
end if;/* dmap converted statement start */
...
if length(v_lista_mails) > 5 then
perform dbms_output.put_line(  concat('V_LISTA_MAILS: ', v_lista_mails , ' V_MAIL_SUBJECT: ', v_mail_subject)) ;/* dmap converted statement end */
call xxmor_funcional_pkg_xxmor_html_email_pr(v_lista_mails, v_mail_subject, null, xxmor_funcional_pkg_xxmor_html_mail(p_id_solicitud, p_lineas, 10)  );
end if;/* dmap converted statement start */
for c_mails in select * from add_buyunt_mkt_cur(p_id_solicitud) loop
v_lista_mails_mkt :=  concat(v_lista_mails_mkt, c_mails.mails) ;/* dmap converted statement end *//* dmap converted statement start */
perform dbms_output.put_line(  concat('C_MAILS.MAILS: ', c_mails.mails)) ;/* dmap converted statement end */
