create or replace procedure labconf."sp_nmlstnoma"  (ws_nom_rep varchar,ws_des_rep varchar,ws_ide_pcc varchar,wn_key_usu numeric,ws_key_men varchar,ws_hor_reg varchar,wn_key_pro numeric,wn_tip_nom numeric,ws_key_per varchar ) as $body$
declare
...
where pam_keypar = (select pam_folini from labconf.glcopams where pam_keypar = '00' and pam_cvesec = 'calfin')
...
ws_opc_009:= null;
begin select pam_folini
into strict ws_opc_009 from labconf.glcopams
where pam_keypar = (
select pam_folini from labconf.glcopams
where pam_keypar='00'
and pam_cvesec='repnom')
...
wn_dsp_009:=0;
wn_dsp_010:=0;
call labconf.sp_glnewdsp ('nmwkmovt', 'mov_keyemp', ws_key_men,wn_dsp_001);
call labconf.sp_glnewdsp ('nmwkmovt', 'mov_keydep', ws_key_men,wn_dsp_002);
call labconf.sp_glnewdsp ('nmwkmovt', 'mov_keypue', ws_key_men,wn_dsp_003);
call labconf.sp_glnewdsp ('nmwkmovt', 'mov_keycon', ws_key_men,wn_dsp_004);
call labconf.sp_glnewdsp ('nmwkmovt', 'mov_keyper', ws_key_men,wn_dsp_005);
call labconf.sp_glnewdsp ('nmwkmovt', 'mov_keynom', ws_key_men,wn_dsp_006);
call labconf.sp_glnewdsp ('nmwkmovt', 'mov_cantid', ws_key_men,wn_dsp_007);
call labconf.sp_glnewdsp ('nmwkmovt', 'mov_import', ws_key_men,wn_dsp_008);
call labconf.sp_glnewdsp ('nmwkmovt', 'emp_tipemp', ws_key_men,wn_dsp_009);
call labconf.sp_glnewdsp ('nmwkmovt', 'emp_keyloc', ws_key_men,wn_dsp_010);
wn_num_reg:=0;
wn_pct_act:=1;
...
into strict wd_fec_ini, wd_fec_fin from labconf.nmloperi
...
k:=1;
for c_nmlstnom in ( select mov_keyemp, mov_keycon, mov_cantid, mov_import, mov_codimp,mov_fecmov from labconf.nmwkmovt
where mov_keyemp in (
...
into strict ws_nom_emp, ws_reg_rfc, wn_sal_dia, wn_sal_int, ws_key_dep, ws_key_pue, ws_tip_emp, ws_key_ims, ws_key_loc, ws_reg_ims,wd_fec_antig,wd_fec_baj,ws_rec_urp,wd_fec_ing,wd_fec_rei from labconf.nmcoempl
...
into strict ws_dep_his, ws_pue_his, ws_loc_his from labconf.nmwkmovt
...
if (k=3 ) then
update labconf.glwkcrys set cry_chr038=ws_key_con,cry_dec015=wn_imp_mov,cry_chr003=ws_des_con
where cry_nomrep=ws_nom_rep
and cry_idepcc=ws_ide_pcc
and cry_keyusu=wn_key_usu
and cry_numsec=-1
