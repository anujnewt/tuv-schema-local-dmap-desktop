CREATE OR REPLACE NONEDITIONABLE PROCEDURE "LABCONF"."SP_I_VACACIONES" 
(o_num_r_procesados out integer, o_num_r_insertados out integer,
...
--Igneos.I
  OPEN cursor1(fFechaProceso);
	...
	--Lleva la cuenta de los registros procesados
		...
		SELECT pam_folfin INTO c_tab_vacaciones
  ...
  --FOREACH
     ...
     BEGIN
          SELECT tab_eledos INTO n_r_anio_trab FROM nmlotabn
          WHERE tab_keytab= trim(c_tab_vacaciones) And tab_eledos >= n_anio_trabajados and ROWNUM = 1 ORDER BY tab_eledos;
     EXCEPTION
