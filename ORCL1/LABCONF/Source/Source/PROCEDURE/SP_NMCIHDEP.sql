CREATE OR REPLACE NONEDITIONABLE PROCEDURE "LABCONF"."SP_NMCIHDEP" (ws_nom_rep IN VARCHAR2,ws_ide_pcc IN VARCHAR2,ws_key_usu IN  NUMBER,ws_key_men IN VARCHAR2,ws_hor_reg IN VARCHAR2,ws_lis_per IN VARCHAR2,ws_dep_pri IN VARCHAR2,ws_key_est IN VARCHAR2 ) IS
...
EXCEPTION
 WHEN NO_DATA_FOUND THEN
  ...
  BEGIN SELECT  COUNT( DISTINCT his_keyemp ) alias1
	INTO wn_mov_emp FROM LABCONF.nmlohism, LABCONF.eocorede
...
EXCEPTION
 WHEN NO_DATA_FOUND THEN
  ...
  ws_lis_pro:= RTRIM(LTRIM(ws_lis_pro)) ||  RTRIM(LTRIM(ws_lis_pr2)) ||  RTRIM(LTRIM(ws_lis_pr3));
	...
	BEGIN SELECT  COUNT( DISTINCT his_keyemp ) alias1
	INTO wn_tot_emp FROM LABCONF.nmlohism, LABCONF.eocorede
