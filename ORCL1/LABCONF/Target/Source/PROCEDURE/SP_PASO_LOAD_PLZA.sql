CREATE OR REPLACE NONEDITIONABLE PROCEDURE "LABCONF"."SP_PASO_LOAD_PLZA" (keyplz integer) AS
...
BEGIN
BEGIN
SELECT trim(plz_keyest), trim(plz_keydep), trim(plz_keypue), trim(plz_keyplz), trim(plz_keycat),
       trim(plz_keyloc), trim(plz_tippla), trim(plz_fecini), trim(plz_fecfin), trim(plz_diavig),
       trim(plz_turnop), trim(plz_keyhor), trim(plz_keyemp), trim(plz_cveuoc), trim(plz_cverem),
       trim(plz_fecmov), trim(plz_submov), trim(plz_cosplz), trim(plz_ca1aux), trim(plz_ca2aux),
       trim(plz_ca3aux), trim(plz_nu1aux), trim(plz_nu2aux), trim(plz_nu3aux), trim(plz_fe1aux),
       trim(plz_fe2aux), trim(plz_fe3aux), trim(plz_co1aux), trim(plz_co2aux), trim(plz_co3aux),
       trim(plz_co4aux), trim(plz_co5aux), trim(plz_keysue), trim(plz_sueniv), trim(plz_subniv),
       trim(plz_cobert), trim(plz_keypro), trim(plz_keydpl), trim(plz_fecocu), trim(plz_salplz),
       trim(plz_titula), trim(plz_origen), trim(plz_valimp), trim(plz_limocu), trim(plz_tiptab)
...
SELECT count(*)
  ...
  NULL;
  END;
IF vg_fecfin is not null
  and vn_existr <> 0  and vg_keyplz is not null  THEN
   DELETE FROM  eocoplza
     WHERE plz_keyplz = keyplz;
ELSE
   IF vn_existr = 0 and vg_keyplz is not null THEN
      INSERT INTO eocoplza
