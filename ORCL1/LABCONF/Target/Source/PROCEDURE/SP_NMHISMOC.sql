CREATE OR REPLACE NONEDITIONABLE PROCEDURE "LABCONF"."SP_NMHISMOC" (ws_nom_rep IN VARCHAR2,ws_ide_pcc IN VARCHAR2,wn_key_usu IN  NUMBER,ws_key_men IN VARCHAR2,ws_hor_reg IN VARCHAR2,ws_etq_rep IN VARCHAR2 ) IS
...
--/GSA
	...
	END LOOP;
	ws_etq_006:='DESCRIP';
	BEGIN SELECT cam_descor	INTO ws_etq_006 FROM LABCONF.glcocamp	WHERE cam_keytab='nmloconc'
    ...
    END;
	ws_etq_008:='NOMBRE';
	BEGIN SELECT cam_descor	INTO ws_etq_008 FROM LABCONF.glcocamp	WHERE cam_keytab='nmcoempl'
	...
	LABCONF.sp_glNewDsp('nmlohism', 'his_keycon', ws_key_men,wn_dsp_002);
	LABCONF.sp_glNewDsp('nmlohism', 'his_cantid', ws_key_men,wn_dsp_003);
...
--	SELECT lis_deslis	INTO ws_var_xxx FROM glcolist WHERE lis_keylis = ws_etq_rep;
	BEGIN SELECT lis_deslis	INTO ws_var_xxx FROM LABCONF.glcolist WHERE lis_keylis = ws_etq_rep;
		      EXCEPTION
			  WHEN NO_DATA_FOUND THEN
			  	     NULL;
    END;
...
--</gsa>
	...
	IF (ws_key_con != ws_con_ant ) THEN
		BEGIN SELECT con_descon	INTO ws_des_con FROM LABCONF.nmloconc	WHERE con_keycon=ws_key_con;
				      EXCEPTION
			  WHEN NO_DATA_FOUND THEN
			  	     NULL;
        END;
	...
	BEGIN SELECT pro_despro	INTO ws_des_pro FROM LABCONF.nmloproc	WHERE pro_keypro=wn_key_pro;
			      EXCEPTION
			  WHEN NO_DATA_FOUND THEN
			  	     NULL;
    END;
	IF (wn_emp_ant!=wn_key_emp ) THEN
		...
		BEGIN SELECT emp_nomemp INTO ws_nom_emp FROM LABCONF.nmcoempl WHERE emp_keyemp = wn_key_emp;
		      EXCEPTION
			  WHEN NO_DATA_FOUND THEN
			  	     NULL;
        END;
