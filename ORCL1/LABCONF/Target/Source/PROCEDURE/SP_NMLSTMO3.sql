CREATE OR REPLACE NONEDITIONABLE PROCEDURE "LABCONF"."SP_NMLSTMO3" (ws_nom_rep IN VARCHAR2,ws_ide_pcc IN VARCHAR2,wn_key_usu IN  NUMBER,ws_key_men IN VARCHAR2,ws_hor_reg IN VARCHAR2,wn_key_pro IN NUMBER,ws_key_per IN VARCHAR2,wn_tip_rep IN NUMBER,ws_nom_rp1 IN VARCHAR2 ) IS
...
-- PGV moved types end
    ...
    wn_key_nom INTEGER;
		BEGIN
	LABCONF.sp_glNewdsp('nmwkmovt', 'mov_cantid', ws_key_men,wn_dsp_001);
	LABCONF.sp_glNewdsp('nmwkmovt', 'mov_import', ws_key_men,wn_dsp_002);
...
EXCEPTION
 WHEN NO_DATA_FOUND THEN
  ...
  ws_opc_009:='';
	BEGIN
    SELECT pam_folini INTO ws_opc_009 FROM LABCONF.glcopams
      WHERE pam_keypar = (SELECT pam_folini FROM LABCONF.glcopams WHERE pam_keypar='00' AND pam_cvesec='repnom')
  ...
  FOR c_nmlstcic IN ( SELECT mov_keyemp, mov_keycon, mov_cantid, mov_import, mov_codimp,mov_fecmov,mov_keynom FROM nmwkmovt
	...
	INTO ws_nom_emp,wd_fec_antig,wd_fec_baj FROM LABCONF.nmcoempl
...
EXCEPTION
 WHEN NO_DATA_FOUND THEN
  ...
  END IF;
	...
	wn_imp_ort:=NULL;
	END IF;
		INSERT INTO LABCONF.glwkcrys( cry_nomrep,cry_idepcc,cry_keyusu,cry_chr001,cry_chr017,cry_chr003,cry_dec006,cry_chr002,cry_dec001,cry_dec002,cry_chr006,cry_chr007,cry_chr008,cry_chr009,cry_chr010,cry_chr011,cry_dat001,cry_chr024,cry_chr025,cry_chr004,cry_dec003,cry_dec004,cry_chr005,cry_chr018,cry_chr016,cry_dec013,cry_dat004,cry_dat005)
