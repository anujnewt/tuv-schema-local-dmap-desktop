CREATE OR REPLACE NONEDITIONABLE PROCEDURE "LABCONF"."SP_NMLSTNOMA" (ws_nom_rep IN VARCHAR2,ws_des_rep IN VARCHAR2,ws_ide_pcc IN VARCHAR2,wn_key_usu IN  NUMBER,ws_key_men IN VARCHAR2,ws_hor_reg IN VARCHAR2,wn_key_pro IN NUMBER,wn_tip_nom IN NUMBER,ws_key_per IN VARCHAR2 ) IS
...
EXCEPTION
 WHEN NO_DATA_FOUND THEN
  ...
  ws_opc_009:='';
	BEGIN SELECT pam_folini
	INTO ws_opc_009 FROM LABCONF.glcopams
	WHERE pam_keypar = (
	SELECT pam_folini FROM LABCONF.glcopams
	WHERE pam_keypar='00'
	AND pam_cvesec='repnom')
...
EXCEPTION
 WHEN NO_DATA_FOUND THEN
  ...
  ws_etq_001:='........';
	...
	wn_dsp_009:=0;
	wn_dsp_010:=0;
	LABCONF.sp_glnewdsp('nmwkmovt', 'mov_keyemp', ws_key_men,wn_dsp_001);
	LABCONF.sp_glnewdsp('nmwkmovt', 'mov_keydep', ws_key_men,wn_dsp_002);
	LABCONF.sp_glnewdsp('nmwkmovt', 'mov_keypue', ws_key_men,wn_dsp_003);
	LABCONF.sp_glnewdsp('nmwkmovt', 'mov_keycon', ws_key_men,wn_dsp_004);
	LABCONF.sp_glnewdsp('nmwkmovt', 'mov_keyper', ws_key_men,wn_dsp_005);
	LABCONF.sp_glnewdsp('nmwkmovt', 'mov_keynom', ws_key_men,wn_dsp_006);
	LABCONF.sp_glnewdsp('nmwkmovt', 'mov_cantid', ws_key_men,wn_dsp_007);
	LABCONF.sp_glnewdsp('nmwkmovt', 'mov_import', ws_key_men,wn_dsp_008);
	LABCONF.sp_glnewdsp('nmwkmovt', 'emp_tipemp', ws_key_men,wn_dsp_009);
	LABCONF.sp_glnewdsp('nmwkmovt', 'emp_keyloc', ws_key_men,wn_dsp_010);
	wn_num_reg:=0;
	wn_pct_act:=1;
...
EXCEPTION
 WHEN NO_DATA_FOUND THEN
  ...
  ws_des_aux:= SUBSTR(ws_des_lis,41,9);
	...
	INTO ws_nom_emp, ws_reg_rfc, wn_sal_dia, wn_sal_int, ws_key_dep, ws_key_pue, ws_tip_emp, ws_key_ims, ws_key_loc, ws_reg_ims,wd_fec_antig,wd_fec_baj,ws_rec_urp,wd_fec_ing,wd_fec_rei FROM LABCONF.nmcoempl
...
EXCEPTION
 WHEN NO_DATA_FOUND THEN
  ...
  ws_dep_his:=NULL;
	...
	INTO ws_dep_his, ws_pue_his, ws_loc_his FROM LABCONF.nmwkmovt
