CREATE OR REPLACE NONEDITIONABLE PROCEDURE "LABCONF"."SP_NMLSTCTT" (ws_nom_rep IN VARCHAR2,ws_ide_pcc IN VARCHAR2,wn_key_usu IN  NUMBER,ws_key_men IN VARCHAR2,ws_hor_reg IN VARCHAR2 ) IS
...
-- PGV moved types end
	...
	WHERE pam_keypar IN (
	SELECT pam_cvesec FROM glcopams
	WHERE pam_keypar='TT')
	AND pam_keypar IN (
	...
	ws_etq_006:='Nada';
	FOR c_nmlstcic IN ( SELECT pam_keypar, pam_cvesec, pam_nompar, pam_folini, pam_folfin FROM glcopams
	WHERE pam_keypar IN (
	SELECT pam_cvesec FROM glcopams
	WHERE pam_keypar='TT')
	AND pam_keypar IN (
	...
	ws_pam_fin:=NULL;
	END IF;
		INSERT INTO glwkcrys( cry_nomrep,cry_idepcc,cry_keyusu,cry_chr001,cry_chr002,cry_chr014,cry_chr004,cry_chr005,cry_chr006,cry_chr007,cry_chr008,cry_chr009,cry_chr010,cry_chr011,cry_chr012,cry_chr013,cry_dat001,cry_chr003)
