CREATE OR REPLACE NONEDITIONABLE PACKAGE "LABCONF"."INTERFACERECIBOS" IS
   ---------------------------------
    PROCEDURE INSERTARESTATUSRECIBO (IDCOMPROBANTEEMP IN NUMBER, STATUS IN NUMBER, ERRORSTR IN VARCHAR2);
    PROCEDURE INSERTAR_EJECUCION(idEjecucion OUT NUMBER);
    PROCEDURE ACTUALIZAR_EJECUCION(idEjecucion IN NUMBER, MENSAJE IN VARCHAR2, ESERROR IN NUMBER);
    PROCEDURE INSERTAR_GENERACION(idGeneracion OUT NUMBER, KEYPRO IN NUMBER, KEYPER IN VARCHAR2);
    PROCEDURE ACTUALIZAR_GENERACION(idGeneracion IN NUMBER, MENSAJE IN VARCHAR2, ESERROR IN NUMBER);
    PROCEDURE RECIBOSTV2016_LIST( KEYPRO IN NUMBER, KEYPER IN VARCHAR2, CV_EMPLIST OUT SYS_REFCURSOR );
    PROCEDURE RECIBOSTV2016_DETAILS(v_KeyBin IN VARCHAR2,KEYPRO IN NUMBER, KEYPER IN VARCHAR2, CV_EMPDETAILS OUT SYS_REFCURSOR );
    PROCEDURE NOMBRE_REPORTE(KEYPRO IN NUMBER, KEYPER IN VARCHAR2, REPORTE OUT VARCHAR2);
END INTERFACERECIBOS;
/
CREATE OR REPLACE NONEDITIONABLE PACKAGE BODY "LABCONF"."INTERFACERECIBOS" IS
    /* INSERTAR REGISTROS EN LA TABLA INSERTARESTATUSRECIBO PARA CADA RECIBO ENVIADO AL PORTAL*/
    /* -------------------------------------------------------------------------- */
    PROCEDURE INSERTARESTATUSRECIBO
    (IDCOMPROBANTEEMP IN NUMBER, STATUS IN NUMBER, ERRORSTR IN VARCHAR2) IS
    BEGIN
        MERGE INTO LABCONF.CFDI2ESTATUSRECIBOS
        USING
            (SELECT INSERTARESTATUSRECIBO.IDCOMPROBANTEEMP IDCOMPROBANTEEMP,
                    SYSDATE EST_FECLEC,
                    INSERTARESTATUSRECIBO.STATUS EST_STATUS,
                    ERRORSTR EST_ERROR  FROM dual) CFDI
        ON
            (LABCONF.CFDI2ESTATUSRECIBOS.IDCOMPROBANTEEMP = INSERTARESTATUSRECIBO.IDCOMPROBANTEEMP)
        WHEN MATCHED THEN
    ...
    END INSERTARESTATUSRECIBO;
    /* -------------------------------------------------------------------------- */
    /* INSERTAR REGISTROS EN LA TABLA RECIBOS_INTERFACE*/
    PROCEDURE INSERTAR_EJECUCION(idEjecucion OUT NUMBER) IS
    BEGIN
        INSERT INTO LABCONF.RECIBOS_INTERFACE (EJE_FECEJE,EJE_MENSAJ,EJE_ERROR)
        VALUES (SYSDATE,'INICIO EJECUCION DE INTERFACE DE RECIBOS',0)
        RETURNING id INTO idEjecucion;
    END INSERTAR_EJECUCION;
    /* -------------------------------------------------------------------------- */
    /* ACTUALIZAR UN REGISTRO EN LA TABLA RECIBOS_INTERFACE*/
    PROCEDURE ACTUALIZAR_EJECUCION(idEjecucion IN NUMBER, MENSAJE IN VARCHAR2, ESERROR IN NUMBER) IS
    BEGIN
        UPDATE LABCONF.RECIBOS_INTERFACE SET
            EJE_FECEJE = SYSDATE,
            EJE_MENSAJ = SUBSTR(ACTUALIZAR_EJECUCION.MENSAJE,1,4000),
            EJE_ERROR = ACTUALIZAR_EJECUCION.ESERROR
        WHERE ID = idEjecucion;
    END ACTUALIZAR_EJECUCION;
    /* -------------------------------------------------------------------------- */
    /* INSERTAR REGISTROS EN LA TABLA RECIBOS_GENERACION*/
    PROCEDURE INSERTAR_GENERACION(idGeneracion OUT NUMBER, KEYPRO IN NUMBER, KEYPER IN VARCHAR2) IS
    BEGIN
        INSERT INTO LABCONF.RECIBOS_GENERACION (EJE_FECEJE,EJE_MENSAJ,EJE_ERROR,EJE_KEYPRO,EJE_KEYPER)
        VALUES (SYSDATE,'INICIO GENERACION DE RECIBOS',0,KEYPRO,KEYPER)
        RETURNING id INTO idGeneracion;
    END INSERTAR_GENERACION;
    /* -------------------------------------------------------------------------- */
    /* ACTUALIZAR UN REGISTRO EN LA TABLA RECIBOS_GENERACION*/
    PROCEDURE ACTUALIZAR_GENERACION(idGeneracion IN NUMBER, MENSAJE IN VARCHAR2, ESERROR IN NUMBER) IS
    BEGIN
        UPDATE LABCONF.RECIBOS_GENERACION SET
            EJE_FECEJE = SYSDATE,
            EJE_MENSAJ = SUBSTR(ACTUALIZAR_GENERACION.MENSAJE,1,4000),
            EJE_ERROR = ACTUALIZAR_GENERACION.ESERROR
        WHERE ID = idGeneracion;
    END ACTUALIZAR_GENERACION;
    /* -------------------------------------------------------------------------- */
    /* GENERAR LA LISTA DE EMPLEADOS QUE VAN A GENERAR RECIBOS*/
    PROCEDURE RECIBOSTV2016_LIST( KEYPRO IN NUMBER, KEYPER IN VARCHAR2, CV_EMPLIST OUT SYS_REFCURSOR ) IS
    /*KEYPRO  Proceso
      KEYPER  Periodo */
    BEGIN
      --OBTENER PARAMETROS
      BEGIN
        ...
        'RN_' || UTILS.CONVERT_TO_VARCHAR2(KEYPRO,4000) || '_' || KEYPER || '_' || UTILS.CONVERT_TO_VARCHAR2(hem_keyemp,4000) || '.PDF' PdfFileName
        FROM LABCONF.nmloperi
        JOIN LABCONF.nmlohemp ON ( hem_keypro = per_keypro AND hem_keyper = per_keyper )
        WHERE per_keypro = KEYPRO AND per_keyper = KEYPER;
      END;
    EXCEPTION
    WHEN OTHERS THEN
      --utils.handleerror(SQLCODE,SQLERRM);
      RETURN;
    END RECIBOSTV2016_LIST;
    /* -------------------------------------------------------------------------- */
    /* GENERAR EL DETALLE DE EMPLEADOS QUE VAN A GENERAR RECIBOS*/
    PROCEDURE RECIBOSTV2016_DETAILS(
        v_KeyBin IN VARCHAR2,
        KEYPRO IN NUMBER, KEYPER IN VARCHAR2,
    ...
    BEGIN
      DECLARE
        ...
        v_KeyPro := KEYPRO;
        v_KeyPer := KEYPER;
        SELECT per_keynom INTO v_KeyNom
          FROM LABCONF.nmloperi
        ...
        ------------------ Clave de OPCIS  ------------------
            ...
            -----------------------------------------------------
            BEGIN
                INSERT INTO LABCONF.PS_TPW_MSGXCONCS
                (SELECT v_KeyPro, v_KeyPer, CON_KEYCON , 1 FROM LABCONF.NMLOCONC WHERE CON_KEYCON IN ( '001' ));
            END;
            -----------------------------------------------------
            -- 2 TOT_ISR
            -----------------------------------------------------
            ...
            EXCEPTION
              WHEN NO_DATA_FOUND THEN
                INSERT INTO LABCONF.PS_TPW_MSGXCONCS
                  (SELECT v_KeyPro, v_KeyPer, CON_KEYCON , 2 FROM LABCONF.NMLOCONC WHERE CON_KEYCON IN ( '100','151','183','294','47A' )
                  );
            END;
            -----------------------------------------------------
            -- 3 TOT_PER
            ...
            EXCEPTION
              WHEN NO_DATA_FOUND THEN
              INSERT INTO LABCONF.PS_TPW_MSGXCONCS
              (SELECT v_KeyPro, v_KeyPer, CON_KEYCON, 3 FROM LABCONF.NMLOCONC  WHERE CON_KEYCON IN ( '260','273','290' ));
            END;
            -----------------------------------------------------
            -- 4 TOT_DED
            -----------------------------------------------------
            INSERT
            INTO LABCONF.PS_TPW_MSGXCONCS
              (SELECT v_KeyPro, v_KeyPer, CON_KEYCON, 4 FROM LABCONF.NMLOCONC
            ...
            EXCEPTION
              ...
              INSERT
              INTO LABCONF.PS_TPW_MSGXCONCS
                ( SELECT v_KeyPro, v_KeyPer, CON_KEYCON , 6 FROM LABCONF.NMLOCONC WHERE CON_KEYCON IN ( 'D63','23D' ));
            END;
            -----------------------------------------------------
            ...
            EXCEPTION
              ...
              INSERT
              INTO LABCONF.PS_TPW_MSGXCONCS
                ( SELECT v_KeyPro, v_KeyPer, CON_KEYCON , 8 FROM LABCONF.NMLOCONC WHERE CON_KEYCON IN ( '28D' ));
            END;
            -----------------------------------------------------
            ...
            EXCEPTION
              ...
              INSERT
              INTO LABCONF.PS_TPW_MSGXCONCS
                ( SELECT v_KeyPro, v_KeyPer, CON_KEYCON , 9 FROM LABCONF.NMLOCONC WHERE CON_KEYCON IN ( '423' ));
            END;
             -----------------------------------------------------
            ...
            INSERT
            INTO LABCONF.PS_TPW_MSGXCONCS
              (SELECT v_KeyPro, v_KeyPer, CON_KEYCON, 10 FROM LABCONF.NMLOCONC
            ...
            INSERT
            INTO LABCONF.PS_TPW_MSGXCONCS
              (SELECT v_KeyPro, v_KeyPer, CON_KEYCON, 11 FROM LABCONF.NMLOCONC
        ...
        IF v_KeyNom = 25 THEN
            ...
            OPEN CV_EMPDETAILS FOR
              ...
              SELECT his_keyemp, SEC_ROW,
                ...
                SUM(Adeudo) Adeudo
              FROM
                (   --ESTA ES LA DIFERENCIA DEL QUERY ENTRE LA NÓMINA DE AGUINALDO Y LA NOMINA ORDINARIA
        ...
        END IF;
        END;
    END RECIBOSTV2016_DETAILS;
    /* -------------------------------------------------------------------------- */
    /* OBTENER EL NOMBRE DEL ARCHIVO RPT*/
    PROCEDURE NOMBRE_REPORTE(KEYPRO IN NUMBER, KEYPER IN VARCHAR2, REPORTE OUT VARCHAR2)
    AS
    KEYNOM NUMBER(10,0) := NULL;
    BEGIN
        REPORTE := 'reciboTV10.rpt';
        SELECT per_keynom INTO KEYNOM
        FROM LABCONF.NMLOPERI
