create or replace procedure labconf.tvcontab_sp_reclasifica2 (proceso labconf.nmlohism.his_keypro%type, periodo labconf.nmlohism.his_keyper%type,keypol labconf.tvwkpoli.pol_keypol%type) as $body$
declare
...
if dmap_extension.f_dmap_get_pkg_var('LABCONF' , 'TVCONTAB', 'PROCESO', 'number', 'N')::numeric not in (533,534,535,553,560) then
return;
end if;
select cia_ca3aux into strict ws_cia from labconf.nmloproc
inner join labconf.nmlocias on cia_keycia = pro_keycia
where pro_keypro = dmap_extension.f_dmap_get_pkg_var('LABCONF' , 'TVCONTAB', 'PROCESO', 'number', 'N')::numeric;
